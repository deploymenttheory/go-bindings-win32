...
func (self *ISpEnginePronunciation) Normalize(pszWord string, pszLeftContext string, pszRightContext string, LangID uint16, pNormalizationList *SPNORMALIZATIONLIST) error {
...
func (self *ISpEnginePronunciation) GetPronunciations(pszWord string, pszLeftContext string, pszRightContext string, LangID uint16, pEnginePronunciationList *SPWORDPRONUNCIATIONLIST) error {
	_pszWord := win32.UTF16Ptr(pszWord)
	_pszLeftContext := win32.UTF16Ptr(pszLeftContext)
	_pszRightContext := win32.UTF16Ptr(pszRightContext)
...
func (self *ISpGrammarBuilder) GetRule(pszRuleName string, dwRuleId uint32, dwAttributes uint32, fCreateIfNotExist bool, phInitialState *SPSTATEHANDLE) error {
...
func (self *ISpGrammarBuilder) AddWordTransition(hFromState SPSTATEHANDLE, hToState SPSTATEHANDLE, psz string, pszSeparators string, eWordType SPGRAMMARWORDTYPE, Weight float32, pPropInfo *SPPROPERTYINFO) error {
	...
	_pszSeparators := win32.UTF16Ptr(pszSeparators)
...
func (self *ISpRecoContext2) SetAdaptationData2(pAdaptationData string, cch uint32, pTopicName string, eAdaptationSettings uint32, eRelevance SPADAPTATIONRELEVANCE) error {
	_pAdaptationData := win32.UTF16Ptr(pAdaptationData)
...
func (self *ISpRecoGrammar) LoadCmdFromResource(hModule foundation.HMODULE, pszResourceName string, pszResourceType string, wLanguage uint16, Options SPLOADOPTIONS) error {
	_pszResourceName := win32.UTF16Ptr(pszResourceName)
...
func (self *ISpSREngine) SetPropertyString(eSrc SPPROPSRC, pvSrcObj unsafe.Pointer, pName string, pValue string) error {
	_pName := win32.UTF16Ptr(pName)
	_pValue := win32.UTF16Ptr(pValue)
...
func (self *ISpSREngine2) SetAdaptationData2(pvEngineContext unsafe.Pointer, pAdaptationData string, cch uint32, pTopicName string, eSettings SPADAPTATIONSETTINGS, eRelevance SPADAPTATIONRELEVANCE) error {
	_pAdaptationData := win32.UTF16Ptr(pAdaptationData)
	_pTopicName := win32.UTF16Ptr(pTopicName)
...
func (self *ISpShortcut) AddShortcut(pszDisplay string, LangID uint16, pszSpoken string, shType SPSHORTCUTTYPE) error {
...
func (self *ISpShortcut) RemoveShortcut(pszDisplay string, LangID uint16, pszSpoken string, shType SPSHORTCUTTYPE) error {
	_pszDisplay := win32.UTF16Ptr(pszDisplay)
	_pszSpoken := win32.UTF16Ptr(pszSpoken)
