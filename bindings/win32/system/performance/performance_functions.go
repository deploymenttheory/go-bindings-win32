// Code generated by go-bindings-win32-codegen. DO NOT EDIT.

//go:build windows && (amd64 || arm64)

package performance

import (
	"syscall"
	"unsafe"

	"github.com/deploymenttheory/go-bindings-win32/bindings/runtime/win32"
	"github.com/deploymenttheory/go-bindings-win32/bindings/win32/foundation"
)

var (
	modADVAPI32 = win32.NewDLL("ADVAPI32.dll")
	modKERNEL32 = win32.NewDLL("KERNEL32.dll")
	modloadperf = win32.NewDLL("loadperf.dll")
	modpdh      = win32.NewDLL("pdh.dll")
)

var (
	procPerfAddCounters                     = modADVAPI32.NewProc("PerfAddCounters")
	procPerfCloseQueryHandle                = modADVAPI32.NewProc("PerfCloseQueryHandle")
	procPerfCreateInstance                  = modADVAPI32.NewProc("PerfCreateInstance")
	procPerfDecrementULongCounterValue      = modADVAPI32.NewProc("PerfDecrementULongCounterValue")
	procPerfDecrementULongLongCounterValue  = modADVAPI32.NewProc("PerfDecrementULongLongCounterValue")
	procPerfDeleteCounters                  = modADVAPI32.NewProc("PerfDeleteCounters")
	procPerfDeleteInstance                  = modADVAPI32.NewProc("PerfDeleteInstance")
	procPerfEnumerateCounterSet             = modADVAPI32.NewProc("PerfEnumerateCounterSet")
	procPerfEnumerateCounterSetInstances    = modADVAPI32.NewProc("PerfEnumerateCounterSetInstances")
	procPerfIncrementULongCounterValue      = modADVAPI32.NewProc("PerfIncrementULongCounterValue")
	procPerfIncrementULongLongCounterValue  = modADVAPI32.NewProc("PerfIncrementULongLongCounterValue")
	procPerfOpenQueryHandle                 = modADVAPI32.NewProc("PerfOpenQueryHandle")
	procPerfQueryCounterData                = modADVAPI32.NewProc("PerfQueryCounterData")
	procPerfQueryCounterInfo                = modADVAPI32.NewProc("PerfQueryCounterInfo")
	procPerfQueryCounterSetRegistrationInfo = modADVAPI32.NewProc("PerfQueryCounterSetRegistrationInfo")
	procPerfQueryInstance                   = modADVAPI32.NewProc("PerfQueryInstance")
	procPerfSetCounterRefValue              = modADVAPI32.NewProc("PerfSetCounterRefValue")
	procPerfSetCounterSetInfo               = modADVAPI32.NewProc("PerfSetCounterSetInfo")
	procPerfSetULongCounterValue            = modADVAPI32.NewProc("PerfSetULongCounterValue")
	procPerfSetULongLongCounterValue        = modADVAPI32.NewProc("PerfSetULongLongCounterValue")
	procPerfStartProvider                   = modADVAPI32.NewProc("PerfStartProvider")
	procPerfStartProviderEx                 = modADVAPI32.NewProc("PerfStartProviderEx")
	procPerfStopProvider                    = modADVAPI32.NewProc("PerfStopProvider")
	procQueryPerformanceCounter             = modKERNEL32.NewProc("QueryPerformanceCounter")
	procQueryPerformanceFrequency           = modKERNEL32.NewProc("QueryPerformanceFrequency")
	procBackupPerfRegistryToFileW           = modloadperf.NewProc("BackupPerfRegistryToFileW")
	procInstallPerfDll                      = modloadperf.NewProc("InstallPerfDllW")
	procInstallPerfDllA                     = modloadperf.NewProc("InstallPerfDllA")
	procLoadPerfCounterTextStrings          = modloadperf.NewProc("LoadPerfCounterTextStringsW")
	procLoadPerfCounterTextStringsA         = modloadperf.NewProc("LoadPerfCounterTextStringsA")
	procRestorePerfRegistryFromFileW        = modloadperf.NewProc("RestorePerfRegistryFromFileW")
	procSetServiceAsTrusted                 = modloadperf.NewProc("SetServiceAsTrustedW")
	procSetServiceAsTrustedA                = modloadperf.NewProc("SetServiceAsTrustedA")
	procUnloadPerfCounterTextStrings        = modloadperf.NewProc("UnloadPerfCounterTextStringsW")
	procUnloadPerfCounterTextStringsA       = modloadperf.NewProc("UnloadPerfCounterTextStringsA")
	procUpdatePerfNameFiles                 = modloadperf.NewProc("UpdatePerfNameFilesW")
	procUpdatePerfNameFilesA                = modloadperf.NewProc("UpdatePerfNameFilesA")
	procPdhAddCounter                       = modpdh.NewProc("PdhAddCounterW")
	procPdhAddCounterA                      = modpdh.NewProc("PdhAddCounterA")
	procPdhAddEnglishCounter                = modpdh.NewProc("PdhAddEnglishCounterW")
	procPdhAddEnglishCounterA               = modpdh.NewProc("PdhAddEnglishCounterA")
	procPdhBindInputDataSource              = modpdh.NewProc("PdhBindInputDataSourceW")
	procPdhBindInputDataSourceA             = modpdh.NewProc("PdhBindInputDataSourceA")
	procPdhBrowseCounters                   = modpdh.NewProc("PdhBrowseCountersW")
	procPdhBrowseCountersA                  = modpdh.NewProc("PdhBrowseCountersA")
	procPdhBrowseCountersH                  = modpdh.NewProc("PdhBrowseCountersHW")
	procPdhBrowseCountersHA                 = modpdh.NewProc("PdhBrowseCountersHA")
	procPdhCalculateCounterFromRawValue     = modpdh.NewProc("PdhCalculateCounterFromRawValue")
	procPdhCloseLog                         = modpdh.NewProc("PdhCloseLog")
	procPdhCloseQuery                       = modpdh.NewProc("PdhCloseQuery")
	procPdhCollectQueryData                 = modpdh.NewProc("PdhCollectQueryData")
	procPdhCollectQueryDataEx               = modpdh.NewProc("PdhCollectQueryDataEx")
	procPdhCollectQueryDataWithTime         = modpdh.NewProc("PdhCollectQueryDataWithTime")
	procPdhComputeCounterStatistics         = modpdh.NewProc("PdhComputeCounterStatistics")
	procPdhConnectMachine                   = modpdh.NewProc("PdhConnectMachineW")
	procPdhConnectMachineA                  = modpdh.NewProc("PdhConnectMachineA")
	procPdhCreateSQLTables                  = modpdh.NewProc("PdhCreateSQLTablesW")
	procPdhCreateSQLTablesA                 = modpdh.NewProc("PdhCreateSQLTablesA")
	procPdhEnumLogSetNames                  = modpdh.NewProc("PdhEnumLogSetNamesW")
	procPdhEnumLogSetNamesA                 = modpdh.NewProc("PdhEnumLogSetNamesA")
	procPdhEnumMachines                     = modpdh.NewProc("PdhEnumMachinesW")
	procPdhEnumMachinesA                    = modpdh.NewProc("PdhEnumMachinesA")
	procPdhEnumMachinesH                    = modpdh.NewProc("PdhEnumMachinesHW")
	procPdhEnumMachinesHA                   = modpdh.NewProc("PdhEnumMachinesHA")
	procPdhEnumObjectItems                  = modpdh.NewProc("PdhEnumObjectItemsW")
	procPdhEnumObjectItemsA                 = modpdh.NewProc("PdhEnumObjectItemsA")
	procPdhEnumObjectItemsH                 = modpdh.NewProc("PdhEnumObjectItemsHW")
	procPdhEnumObjectItemsHA                = modpdh.NewProc("PdhEnumObjectItemsHA")
	procPdhEnumObjects                      = modpdh.NewProc("PdhEnumObjectsW")
	procPdhEnumObjectsA                     = modpdh.NewProc("PdhEnumObjectsA")
	procPdhEnumObjectsH                     = modpdh.NewProc("PdhEnumObjectsHW")
	procPdhEnumObjectsHA                    = modpdh.NewProc("PdhEnumObjectsHA")
	procPdhExpandCounterPath                = modpdh.NewProc("PdhExpandCounterPathW")
	procPdhExpandCounterPathA               = modpdh.NewProc("PdhExpandCounterPathA")
	procPdhExpandWildCardPath               = modpdh.NewProc("PdhExpandWildCardPathW")
	procPdhExpandWildCardPathA              = modpdh.NewProc("PdhExpandWildCardPathA")
	procPdhExpandWildCardPathH              = modpdh.NewProc("PdhExpandWildCardPathHW")
	procPdhExpandWildCardPathHA             = modpdh.NewProc("PdhExpandWildCardPathHA")
	procPdhFormatFromRawValue               = modpdh.NewProc("PdhFormatFromRawValue")
	procPdhGetCounterInfo                   = modpdh.NewProc("PdhGetCounterInfoW")
	procPdhGetCounterInfoA                  = modpdh.NewProc("PdhGetCounterInfoA")
	procPdhGetCounterTimeBase               = modpdh.NewProc("PdhGetCounterTimeBase")
	procPdhGetDataSourceTimeRange           = modpdh.NewProc("PdhGetDataSourceTimeRangeW")
	procPdhGetDataSourceTimeRangeA          = modpdh.NewProc("PdhGetDataSourceTimeRangeA")
	procPdhGetDataSourceTimeRangeH          = modpdh.NewProc("PdhGetDataSourceTimeRangeH")
	procPdhGetDefaultPerfCounter            = modpdh.NewProc("PdhGetDefaultPerfCounterW")
	procPdhGetDefaultPerfCounterA           = modpdh.NewProc("PdhGetDefaultPerfCounterA")
	procPdhGetDefaultPerfCounterH           = modpdh.NewProc("PdhGetDefaultPerfCounterHW")
	procPdhGetDefaultPerfCounterHA          = modpdh.NewProc("PdhGetDefaultPerfCounterHA")
	procPdhGetDefaultPerfObject             = modpdh.NewProc("PdhGetDefaultPerfObjectW")
	procPdhGetDefaultPerfObjectA            = modpdh.NewProc("PdhGetDefaultPerfObjectA")
	procPdhGetDefaultPerfObjectH            = modpdh.NewProc("PdhGetDefaultPerfObjectHW")
	procPdhGetDefaultPerfObjectHA           = modpdh.NewProc("PdhGetDefaultPerfObjectHA")
	procPdhGetDllVersion                    = modpdh.NewProc("PdhGetDllVersion")
	procPdhGetFormattedCounterArray         = modpdh.NewProc("PdhGetFormattedCounterArrayW")
	procPdhGetFormattedCounterArrayA        = modpdh.NewProc("PdhGetFormattedCounterArrayA")
	procPdhGetFormattedCounterValue         = modpdh.NewProc("PdhGetFormattedCounterValue")
	procPdhGetLogFileSize                   = modpdh.NewProc("PdhGetLogFileSize")
	procPdhGetLogSetGUID                    = modpdh.NewProc("PdhGetLogSetGUID")
	procPdhGetRawCounterArray               = modpdh.NewProc("PdhGetRawCounterArrayW")
	procPdhGetRawCounterArrayA              = modpdh.NewProc("PdhGetRawCounterArrayA")
	procPdhGetRawCounterValue               = modpdh.NewProc("PdhGetRawCounterValue")
	procPdhIsRealTimeQuery                  = modpdh.NewProc("PdhIsRealTimeQuery")
	procPdhLookupPerfIndexByName            = modpdh.NewProc("PdhLookupPerfIndexByNameW")
	procPdhLookupPerfIndexByNameA           = modpdh.NewProc("PdhLookupPerfIndexByNameA")
	procPdhLookupPerfNameByIndex            = modpdh.NewProc("PdhLookupPerfNameByIndexW")
	procPdhLookupPerfNameByIndexA           = modpdh.NewProc("PdhLookupPerfNameByIndexA")
	procPdhMakeCounterPath                  = modpdh.NewProc("PdhMakeCounterPathW")
	procPdhMakeCounterPathA                 = modpdh.NewProc("PdhMakeCounterPathA")
	procPdhOpenLog                          = modpdh.NewProc("PdhOpenLogW")
	procPdhOpenLogA                         = modpdh.NewProc("PdhOpenLogA")
	procPdhOpenQuery                        = modpdh.NewProc("PdhOpenQueryW")
	procPdhOpenQueryA                       = modpdh.NewProc("PdhOpenQueryA")
	procPdhOpenQueryH                       = modpdh.NewProc("PdhOpenQueryH")
	procPdhParseCounterPath                 = modpdh.NewProc("PdhParseCounterPathW")
	procPdhParseCounterPathA                = modpdh.NewProc("PdhParseCounterPathA")
	procPdhParseInstanceName                = modpdh.NewProc("PdhParseInstanceNameW")
	procPdhParseInstanceNameA               = modpdh.NewProc("PdhParseInstanceNameA")
	procPdhReadRawLogRecord                 = modpdh.NewProc("PdhReadRawLogRecord")
	procPdhRemoveCounter                    = modpdh.NewProc("PdhRemoveCounter")
	procPdhSelectDataSource                 = modpdh.NewProc("PdhSelectDataSourceW")
	procPdhSelectDataSourceA                = modpdh.NewProc("PdhSelectDataSourceA")
	procPdhSetCounterScaleFactor            = modpdh.NewProc("PdhSetCounterScaleFactor")
	procPdhSetDefaultRealTimeDataSource     = modpdh.NewProc("PdhSetDefaultRealTimeDataSource")
	procPdhSetLogSetRunID                   = modpdh.NewProc("PdhSetLogSetRunID")
	procPdhSetQueryTimeRange                = modpdh.NewProc("PdhSetQueryTimeRange")
	procPdhUpdateLog                        = modpdh.NewProc("PdhUpdateLogW")
	procPdhUpdateLogA                       = modpdh.NewProc("PdhUpdateLogA")
	procPdhUpdateLogFileCatalog             = modpdh.NewProc("PdhUpdateLogFileCatalog")
	procPdhValidatePath                     = modpdh.NewProc("PdhValidatePathW")
	procPdhValidatePathA                    = modpdh.NewProc("PdhValidatePathA")
	procPdhValidatePathEx                   = modpdh.NewProc("PdhValidatePathExW")
	procPdhValidatePathExA                  = modpdh.NewProc("PdhValidatePathExA")
	procPdhVerifySQLDB                      = modpdh.NewProc("PdhVerifySQLDBW")
	procPdhVerifySQLDBA                     = modpdh.NewProc("PdhVerifySQLDBA")
)

// Procs exposes this package's lazily resolved exports for availability
// probing: Procs.<Function>.Find() reports nil, or the *win32.ProcError a
// call to <Function> would panic with on this system (an export missing from
// this Windows build, or a DLL that is not installed).
var Procs = struct {
	BackupPerfRegistryToFileW           *win32.Proc
	InstallPerfDll                      *win32.Proc
	InstallPerfDllA                     *win32.Proc
	LoadPerfCounterTextStrings          *win32.Proc
	LoadPerfCounterTextStringsA         *win32.Proc
	PdhAddCounter                       *win32.Proc
	PdhAddCounterA                      *win32.Proc
	PdhAddEnglishCounter                *win32.Proc
	PdhAddEnglishCounterA               *win32.Proc
	PdhBindInputDataSource              *win32.Proc
	PdhBindInputDataSourceA             *win32.Proc
	PdhBrowseCounters                   *win32.Proc
	PdhBrowseCountersA                  *win32.Proc
	PdhBrowseCountersH                  *win32.Proc
	PdhBrowseCountersHA                 *win32.Proc
	PdhCalculateCounterFromRawValue     *win32.Proc
	PdhCloseLog                         *win32.Proc
	PdhCloseQuery                       *win32.Proc
	PdhCollectQueryData                 *win32.Proc
	PdhCollectQueryDataEx               *win32.Proc
	PdhCollectQueryDataWithTime         *win32.Proc
	PdhComputeCounterStatistics         *win32.Proc
	PdhConnectMachine                   *win32.Proc
	PdhConnectMachineA                  *win32.Proc
	PdhCreateSQLTables                  *win32.Proc
	PdhCreateSQLTablesA                 *win32.Proc
	PdhEnumLogSetNames                  *win32.Proc
	PdhEnumLogSetNamesA                 *win32.Proc
	PdhEnumMachines                     *win32.Proc
	PdhEnumMachinesA                    *win32.Proc
	PdhEnumMachinesH                    *win32.Proc
	PdhEnumMachinesHA                   *win32.Proc
	PdhEnumObjectItems                  *win32.Proc
	PdhEnumObjectItemsA                 *win32.Proc
	PdhEnumObjectItemsH                 *win32.Proc
	PdhEnumObjectItemsHA                *win32.Proc
	PdhEnumObjects                      *win32.Proc
	PdhEnumObjectsA                     *win32.Proc
	PdhEnumObjectsH                     *win32.Proc
	PdhEnumObjectsHA                    *win32.Proc
	PdhExpandCounterPath                *win32.Proc
	PdhExpandCounterPathA               *win32.Proc
	PdhExpandWildCardPath               *win32.Proc
	PdhExpandWildCardPathA              *win32.Proc
	PdhExpandWildCardPathH              *win32.Proc
	PdhExpandWildCardPathHA             *win32.Proc
	PdhFormatFromRawValue               *win32.Proc
	PdhGetCounterInfo                   *win32.Proc
	PdhGetCounterInfoA                  *win32.Proc
	PdhGetCounterTimeBase               *win32.Proc
	PdhGetDataSourceTimeRange           *win32.Proc
	PdhGetDataSourceTimeRangeA          *win32.Proc
	PdhGetDataSourceTimeRangeH          *win32.Proc
	PdhGetDefaultPerfCounter            *win32.Proc
	PdhGetDefaultPerfCounterA           *win32.Proc
	PdhGetDefaultPerfCounterH           *win32.Proc
	PdhGetDefaultPerfCounterHA          *win32.Proc
	PdhGetDefaultPerfObject             *win32.Proc
	PdhGetDefaultPerfObjectA            *win32.Proc
	PdhGetDefaultPerfObjectH            *win32.Proc
	PdhGetDefaultPerfObjectHA           *win32.Proc
	PdhGetDllVersion                    *win32.Proc
	PdhGetFormattedCounterArray         *win32.Proc
	PdhGetFormattedCounterArrayA        *win32.Proc
	PdhGetFormattedCounterValue         *win32.Proc
	PdhGetLogFileSize                   *win32.Proc
	PdhGetLogSetGUID                    *win32.Proc
	PdhGetRawCounterArray               *win32.Proc
	PdhGetRawCounterArrayA              *win32.Proc
	PdhGetRawCounterValue               *win32.Proc
	PdhIsRealTimeQuery                  *win32.Proc
	PdhLookupPerfIndexByName            *win32.Proc
	PdhLookupPerfIndexByNameA           *win32.Proc
	PdhLookupPerfNameByIndex            *win32.Proc
	PdhLookupPerfNameByIndexA           *win32.Proc
	PdhMakeCounterPath                  *win32.Proc
	PdhMakeCounterPathA                 *win32.Proc
	PdhOpenLog                          *win32.Proc
	PdhOpenLogA                         *win32.Proc
	PdhOpenQuery                        *win32.Proc
	PdhOpenQueryA                       *win32.Proc
	PdhOpenQueryH                       *win32.Proc
	PdhParseCounterPath                 *win32.Proc
	PdhParseCounterPathA                *win32.Proc
	PdhParseInstanceName                *win32.Proc
	PdhParseInstanceNameA               *win32.Proc
	PdhReadRawLogRecord                 *win32.Proc
	PdhRemoveCounter                    *win32.Proc
	PdhSelectDataSource                 *win32.Proc
	PdhSelectDataSourceA                *win32.Proc
	PdhSetCounterScaleFactor            *win32.Proc
	PdhSetDefaultRealTimeDataSource     *win32.Proc
	PdhSetLogSetRunID                   *win32.Proc
	PdhSetQueryTimeRange                *win32.Proc
	PdhUpdateLog                        *win32.Proc
	PdhUpdateLogA                       *win32.Proc
	PdhUpdateLogFileCatalog             *win32.Proc
	PdhValidatePath                     *win32.Proc
	PdhValidatePathA                    *win32.Proc
	PdhValidatePathEx                   *win32.Proc
	PdhValidatePathExA                  *win32.Proc
	PdhVerifySQLDB                      *win32.Proc
	PdhVerifySQLDBA                     *win32.Proc
	PerfAddCounters                     *win32.Proc
	PerfCloseQueryHandle                *win32.Proc
	PerfCreateInstance                  *win32.Proc
	PerfDecrementULongCounterValue      *win32.Proc
	PerfDecrementULongLongCounterValue  *win32.Proc
	PerfDeleteCounters                  *win32.Proc
	PerfDeleteInstance                  *win32.Proc
	PerfEnumerateCounterSet             *win32.Proc
	PerfEnumerateCounterSetInstances    *win32.Proc
	PerfIncrementULongCounterValue      *win32.Proc
	PerfIncrementULongLongCounterValue  *win32.Proc
	PerfOpenQueryHandle                 *win32.Proc
	PerfQueryCounterData                *win32.Proc
	PerfQueryCounterInfo                *win32.Proc
	PerfQueryCounterSetRegistrationInfo *win32.Proc
	PerfQueryInstance                   *win32.Proc
	PerfSetCounterRefValue              *win32.Proc
	PerfSetCounterSetInfo               *win32.Proc
	PerfSetULongCounterValue            *win32.Proc
	PerfSetULongLongCounterValue        *win32.Proc
	PerfStartProvider                   *win32.Proc
	PerfStartProviderEx                 *win32.Proc
	PerfStopProvider                    *win32.Proc
	QueryPerformanceCounter             *win32.Proc
	QueryPerformanceFrequency           *win32.Proc
	RestorePerfRegistryFromFileW        *win32.Proc
	SetServiceAsTrusted                 *win32.Proc
	SetServiceAsTrustedA                *win32.Proc
	UnloadPerfCounterTextStrings        *win32.Proc
	UnloadPerfCounterTextStringsA       *win32.Proc
	UpdatePerfNameFiles                 *win32.Proc
	UpdatePerfNameFilesA                *win32.Proc
}{
	BackupPerfRegistryToFileW:           procBackupPerfRegistryToFileW,
	InstallPerfDll:                      procInstallPerfDll,
	InstallPerfDllA:                     procInstallPerfDllA,
	LoadPerfCounterTextStrings:          procLoadPerfCounterTextStrings,
	LoadPerfCounterTextStringsA:         procLoadPerfCounterTextStringsA,
	PdhAddCounter:                       procPdhAddCounter,
	PdhAddCounterA:                      procPdhAddCounterA,
	PdhAddEnglishCounter:                procPdhAddEnglishCounter,
	PdhAddEnglishCounterA:               procPdhAddEnglishCounterA,
	PdhBindInputDataSource:              procPdhBindInputDataSource,
	PdhBindInputDataSourceA:             procPdhBindInputDataSourceA,
	PdhBrowseCounters:                   procPdhBrowseCounters,
	PdhBrowseCountersA:                  procPdhBrowseCountersA,
	PdhBrowseCountersH:                  procPdhBrowseCountersH,
	PdhBrowseCountersHA:                 procPdhBrowseCountersHA,
	PdhCalculateCounterFromRawValue:     procPdhCalculateCounterFromRawValue,
	PdhCloseLog:                         procPdhCloseLog,
	PdhCloseQuery:                       procPdhCloseQuery,
	PdhCollectQueryData:                 procPdhCollectQueryData,
	PdhCollectQueryDataEx:               procPdhCollectQueryDataEx,
	PdhCollectQueryDataWithTime:         procPdhCollectQueryDataWithTime,
	PdhComputeCounterStatistics:         procPdhComputeCounterStatistics,
	PdhConnectMachine:                   procPdhConnectMachine,
	PdhConnectMachineA:                  procPdhConnectMachineA,
	PdhCreateSQLTables:                  procPdhCreateSQLTables,
	PdhCreateSQLTablesA:                 procPdhCreateSQLTablesA,
	PdhEnumLogSetNames:                  procPdhEnumLogSetNames,
	PdhEnumLogSetNamesA:                 procPdhEnumLogSetNamesA,
	PdhEnumMachines:                     procPdhEnumMachines,
	PdhEnumMachinesA:                    procPdhEnumMachinesA,
	PdhEnumMachinesH:                    procPdhEnumMachinesH,
	PdhEnumMachinesHA:                   procPdhEnumMachinesHA,
	PdhEnumObjectItems:                  procPdhEnumObjectItems,
	PdhEnumObjectItemsA:                 procPdhEnumObjectItemsA,
	PdhEnumObjectItemsH:                 procPdhEnumObjectItemsH,
	PdhEnumObjectItemsHA:                procPdhEnumObjectItemsHA,
	PdhEnumObjects:                      procPdhEnumObjects,
	PdhEnumObjectsA:                     procPdhEnumObjectsA,
	PdhEnumObjectsH:                     procPdhEnumObjectsH,
	PdhEnumObjectsHA:                    procPdhEnumObjectsHA,
	PdhExpandCounterPath:                procPdhExpandCounterPath,
	PdhExpandCounterPathA:               procPdhExpandCounterPathA,
	PdhExpandWildCardPath:               procPdhExpandWildCardPath,
	PdhExpandWildCardPathA:              procPdhExpandWildCardPathA,
	PdhExpandWildCardPathH:              procPdhExpandWildCardPathH,
	PdhExpandWildCardPathHA:             procPdhExpandWildCardPathHA,
	PdhFormatFromRawValue:               procPdhFormatFromRawValue,
	PdhGetCounterInfo:                   procPdhGetCounterInfo,
	PdhGetCounterInfoA:                  procPdhGetCounterInfoA,
	PdhGetCounterTimeBase:               procPdhGetCounterTimeBase,
	PdhGetDataSourceTimeRange:           procPdhGetDataSourceTimeRange,
	PdhGetDataSourceTimeRangeA:          procPdhGetDataSourceTimeRangeA,
	PdhGetDataSourceTimeRangeH:          procPdhGetDataSourceTimeRangeH,
	PdhGetDefaultPerfCounter:            procPdhGetDefaultPerfCounter,
	PdhGetDefaultPerfCounterA:           procPdhGetDefaultPerfCounterA,
	PdhGetDefaultPerfCounterH:           procPdhGetDefaultPerfCounterH,
	PdhGetDefaultPerfCounterHA:          procPdhGetDefaultPerfCounterHA,
	PdhGetDefaultPerfObject:             procPdhGetDefaultPerfObject,
	PdhGetDefaultPerfObjectA:            procPdhGetDefaultPerfObjectA,
	PdhGetDefaultPerfObjectH:            procPdhGetDefaultPerfObjectH,
	PdhGetDefaultPerfObjectHA:           procPdhGetDefaultPerfObjectHA,
	PdhGetDllVersion:                    procPdhGetDllVersion,
	PdhGetFormattedCounterArray:         procPdhGetFormattedCounterArray,
	PdhGetFormattedCounterArrayA:        procPdhGetFormattedCounterArrayA,
	PdhGetFormattedCounterValue:         procPdhGetFormattedCounterValue,
	PdhGetLogFileSize:                   procPdhGetLogFileSize,
	PdhGetLogSetGUID:                    procPdhGetLogSetGUID,
	PdhGetRawCounterArray:               procPdhGetRawCounterArray,
	PdhGetRawCounterArrayA:              procPdhGetRawCounterArrayA,
	PdhGetRawCounterValue:               procPdhGetRawCounterValue,
	PdhIsRealTimeQuery:                  procPdhIsRealTimeQuery,
	PdhLookupPerfIndexByName:            procPdhLookupPerfIndexByName,
	PdhLookupPerfIndexByNameA:           procPdhLookupPerfIndexByNameA,
	PdhLookupPerfNameByIndex:            procPdhLookupPerfNameByIndex,
	PdhLookupPerfNameByIndexA:           procPdhLookupPerfNameByIndexA,
	PdhMakeCounterPath:                  procPdhMakeCounterPath,
	PdhMakeCounterPathA:                 procPdhMakeCounterPathA,
	PdhOpenLog:                          procPdhOpenLog,
	PdhOpenLogA:                         procPdhOpenLogA,
	PdhOpenQuery:                        procPdhOpenQuery,
	PdhOpenQueryA:                       procPdhOpenQueryA,
	PdhOpenQueryH:                       procPdhOpenQueryH,
	PdhParseCounterPath:                 procPdhParseCounterPath,
	PdhParseCounterPathA:                procPdhParseCounterPathA,
	PdhParseInstanceName:                procPdhParseInstanceName,
	PdhParseInstanceNameA:               procPdhParseInstanceNameA,
	PdhReadRawLogRecord:                 procPdhReadRawLogRecord,
	PdhRemoveCounter:                    procPdhRemoveCounter,
	PdhSelectDataSource:                 procPdhSelectDataSource,
	PdhSelectDataSourceA:                procPdhSelectDataSourceA,
	PdhSetCounterScaleFactor:            procPdhSetCounterScaleFactor,
	PdhSetDefaultRealTimeDataSource:     procPdhSetDefaultRealTimeDataSource,
	PdhSetLogSetRunID:                   procPdhSetLogSetRunID,
	PdhSetQueryTimeRange:                procPdhSetQueryTimeRange,
	PdhUpdateLog:                        procPdhUpdateLog,
	PdhUpdateLogA:                       procPdhUpdateLogA,
	PdhUpdateLogFileCatalog:             procPdhUpdateLogFileCatalog,
	PdhValidatePath:                     procPdhValidatePath,
	PdhValidatePathA:                    procPdhValidatePathA,
	PdhValidatePathEx:                   procPdhValidatePathEx,
	PdhValidatePathExA:                  procPdhValidatePathExA,
	PdhVerifySQLDB:                      procPdhVerifySQLDB,
	PdhVerifySQLDBA:                     procPdhVerifySQLDBA,
	PerfAddCounters:                     procPerfAddCounters,
	PerfCloseQueryHandle:                procPerfCloseQueryHandle,
	PerfCreateInstance:                  procPerfCreateInstance,
	PerfDecrementULongCounterValue:      procPerfDecrementULongCounterValue,
	PerfDecrementULongLongCounterValue:  procPerfDecrementULongLongCounterValue,
	PerfDeleteCounters:                  procPerfDeleteCounters,
	PerfDeleteInstance:                  procPerfDeleteInstance,
	PerfEnumerateCounterSet:             procPerfEnumerateCounterSet,
	PerfEnumerateCounterSetInstances:    procPerfEnumerateCounterSetInstances,
	PerfIncrementULongCounterValue:      procPerfIncrementULongCounterValue,
	PerfIncrementULongLongCounterValue:  procPerfIncrementULongLongCounterValue,
	PerfOpenQueryHandle:                 procPerfOpenQueryHandle,
	PerfQueryCounterData:                procPerfQueryCounterData,
	PerfQueryCounterInfo:                procPerfQueryCounterInfo,
	PerfQueryCounterSetRegistrationInfo: procPerfQueryCounterSetRegistrationInfo,
	PerfQueryInstance:                   procPerfQueryInstance,
	PerfSetCounterRefValue:              procPerfSetCounterRefValue,
	PerfSetCounterSetInfo:               procPerfSetCounterSetInfo,
	PerfSetULongCounterValue:            procPerfSetULongCounterValue,
	PerfSetULongLongCounterValue:        procPerfSetULongLongCounterValue,
	PerfStartProvider:                   procPerfStartProvider,
	PerfStartProviderEx:                 procPerfStartProviderEx,
	PerfStopProvider:                    procPerfStopProvider,
	QueryPerformanceCounter:             procQueryPerformanceCounter,
	QueryPerformanceFrequency:           procQueryPerformanceFrequency,
	RestorePerfRegistryFromFileW:        procRestorePerfRegistryFromFileW,
	SetServiceAsTrusted:                 procSetServiceAsTrusted,
	SetServiceAsTrustedA:                procSetServiceAsTrustedA,
	UnloadPerfCounterTextStrings:        procUnloadPerfCounterTextStrings,
	UnloadPerfCounterTextStringsA:       procUnloadPerfCounterTextStringsA,
	UpdatePerfNameFiles:                 procUpdatePerfNameFiles,
	UpdatePerfNameFilesA:                procUpdatePerfNameFilesA,
}

// BackupPerfRegistryToFileW calls loadperf!BackupPerfRegistryToFileW.
func BackupPerfRegistryToFileW(szFileName string, szCommentString *string) uint32 {
	_szFileName := win32.UTF16Ptr(szFileName)
	_szCommentString := win32.UTF16PtrOrNil(szCommentString)
	r1, _, _ := syscall.SyscallN(procBackupPerfRegistryToFileW.Addr(), uintptr(unsafe.Pointer(_szFileName)), uintptr(unsafe.Pointer(_szCommentString)))
	return uint32(r1)
}

// InstallPerfDll calls loadperf!InstallPerfDllW.
// https://learn.microsoft.com/windows/win32/api/loadperf/nf-loadperf-installperfdllw
func InstallPerfDll(szComputerName *string, lpIniFile string, dwFlags uintptr) uint32 {
	_szComputerName := win32.UTF16PtrOrNil(szComputerName)
	_lpIniFile := win32.UTF16Ptr(lpIniFile)
	r1, _, _ := syscall.SyscallN(procInstallPerfDll.Addr(), uintptr(unsafe.Pointer(_szComputerName)), uintptr(unsafe.Pointer(_lpIniFile)), uintptr(dwFlags))
	return uint32(r1)
}

// InstallPerfDllA calls loadperf!InstallPerfDllA.
// https://learn.microsoft.com/windows/win32/api/loadperf/nf-loadperf-installperfdlla
func InstallPerfDllA(szComputerName foundation.PSTR, lpIniFile foundation.PSTR, dwFlags uintptr) uint32 {
	r1, _, _ := syscall.SyscallN(procInstallPerfDllA.Addr(), uintptr(unsafe.Pointer(szComputerName)), uintptr(unsafe.Pointer(lpIniFile)), uintptr(dwFlags))
	return uint32(r1)
}

// LoadPerfCounterTextStrings calls loadperf!LoadPerfCounterTextStringsW.
// https://learn.microsoft.com/windows/win32/api/loadperf/nf-loadperf-loadperfcountertextstringsw
// Minimum OS: windows5.1.2600.
func LoadPerfCounterTextStrings(lpCommandLine string, bQuietModeArg bool) uint32 {
	_lpCommandLine := win32.UTF16Ptr(lpCommandLine)
	_bQuietModeArg := win32.Bool32(bQuietModeArg)
	r1, _, _ := syscall.SyscallN(procLoadPerfCounterTextStrings.Addr(), uintptr(unsafe.Pointer(_lpCommandLine)), uintptr(_bQuietModeArg))
	return uint32(r1)
}

// LoadPerfCounterTextStringsA calls loadperf!LoadPerfCounterTextStringsA.
// https://learn.microsoft.com/windows/win32/api/loadperf/nf-loadperf-loadperfcountertextstringsa
// Minimum OS: windows5.1.2600.
func LoadPerfCounterTextStringsA(lpCommandLine foundation.PSTR, bQuietModeArg bool) uint32 {
	_bQuietModeArg := win32.Bool32(bQuietModeArg)
	r1, _, _ := syscall.SyscallN(procLoadPerfCounterTextStringsA.Addr(), uintptr(unsafe.Pointer(lpCommandLine)), uintptr(_bQuietModeArg))
	return uint32(r1)
}

// PdhAddCounter calls pdh!PdhAddCounterW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhaddcounterw
// Minimum OS: windows5.1.2600.
func PdhAddCounter(hQuery PDH_HQUERY, szFullCounterPath string, dwUserData uintptr, phCounter *PDH_HCOUNTER) uint32 {
	_szFullCounterPath := win32.UTF16Ptr(szFullCounterPath)
	r1, _, _ := syscall.SyscallN(procPdhAddCounter.Addr(), uintptr(hQuery), uintptr(unsafe.Pointer(_szFullCounterPath)), uintptr(dwUserData), uintptr(unsafe.Pointer(phCounter)))
	return uint32(r1)
}

// PdhAddCounterA calls pdh!PdhAddCounterA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhaddcountera
// Minimum OS: windows5.1.2600.
func PdhAddCounterA(hQuery PDH_HQUERY, szFullCounterPath foundation.PSTR, dwUserData uintptr, phCounter *PDH_HCOUNTER) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhAddCounterA.Addr(), uintptr(hQuery), uintptr(unsafe.Pointer(szFullCounterPath)), uintptr(dwUserData), uintptr(unsafe.Pointer(phCounter)))
	return uint32(r1)
}

// PdhAddEnglishCounter calls pdh!PdhAddEnglishCounterW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhaddenglishcounterw
// Minimum OS: windows6.0.6000.
func PdhAddEnglishCounter(hQuery PDH_HQUERY, szFullCounterPath string, dwUserData uintptr, phCounter *PDH_HCOUNTER) uint32 {
	_szFullCounterPath := win32.UTF16Ptr(szFullCounterPath)
	r1, _, _ := syscall.SyscallN(procPdhAddEnglishCounter.Addr(), uintptr(hQuery), uintptr(unsafe.Pointer(_szFullCounterPath)), uintptr(dwUserData), uintptr(unsafe.Pointer(phCounter)))
	return uint32(r1)
}

// PdhAddEnglishCounterA calls pdh!PdhAddEnglishCounterA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhaddenglishcountera
// Minimum OS: windows6.0.6000.
func PdhAddEnglishCounterA(hQuery PDH_HQUERY, szFullCounterPath foundation.PSTR, dwUserData uintptr, phCounter *PDH_HCOUNTER) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhAddEnglishCounterA.Addr(), uintptr(hQuery), uintptr(unsafe.Pointer(szFullCounterPath)), uintptr(dwUserData), uintptr(unsafe.Pointer(phCounter)))
	return uint32(r1)
}

// PdhBindInputDataSource calls pdh!PdhBindInputDataSourceW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhbindinputdatasourcew
// Minimum OS: windows5.1.2600.
func PdhBindInputDataSource(phDataSource *PDH_HLOG, LogFileNameList *string) uint32 {
	_LogFileNameList := win32.UTF16PtrOrNil(LogFileNameList)
	r1, _, _ := syscall.SyscallN(procPdhBindInputDataSource.Addr(), uintptr(unsafe.Pointer(phDataSource)), uintptr(unsafe.Pointer(_LogFileNameList)))
	return uint32(r1)
}

// PdhBindInputDataSourceA calls pdh!PdhBindInputDataSourceA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhbindinputdatasourcea
// Minimum OS: windows5.1.2600.
func PdhBindInputDataSourceA(phDataSource *PDH_HLOG, LogFileNameList foundation.PSTR) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhBindInputDataSourceA.Addr(), uintptr(unsafe.Pointer(phDataSource)), uintptr(unsafe.Pointer(LogFileNameList)))
	return uint32(r1)
}

// PdhBrowseCounters calls pdh!PdhBrowseCountersW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhbrowsecountersw
// Minimum OS: windows5.1.2600.
func PdhBrowseCounters(pBrowseDlgData *PDH_BROWSE_DLG_CONFIG_W) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhBrowseCounters.Addr(), uintptr(unsafe.Pointer(pBrowseDlgData)))
	return uint32(r1)
}

// PdhBrowseCountersA calls pdh!PdhBrowseCountersA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhbrowsecountersa
// Minimum OS: windows5.1.2600.
func PdhBrowseCountersA(pBrowseDlgData *PDH_BROWSE_DLG_CONFIG_A) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhBrowseCountersA.Addr(), uintptr(unsafe.Pointer(pBrowseDlgData)))
	return uint32(r1)
}

// PdhBrowseCountersH calls pdh!PdhBrowseCountersHW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhbrowsecountershw
// Minimum OS: windows5.1.2600.
func PdhBrowseCountersH(pBrowseDlgData *PDH_BROWSE_DLG_CONFIG_HW) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhBrowseCountersH.Addr(), uintptr(unsafe.Pointer(pBrowseDlgData)))
	return uint32(r1)
}

// PdhBrowseCountersHA calls pdh!PdhBrowseCountersHA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhbrowsecountersha
// Minimum OS: windows5.1.2600.
func PdhBrowseCountersHA(pBrowseDlgData *PDH_BROWSE_DLG_CONFIG_HA) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhBrowseCountersHA.Addr(), uintptr(unsafe.Pointer(pBrowseDlgData)))
	return uint32(r1)
}

// PdhCalculateCounterFromRawValue calls pdh!PdhCalculateCounterFromRawValue.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhcalculatecounterfromrawvalue
// Minimum OS: windows5.1.2600.
func PdhCalculateCounterFromRawValue(hCounter PDH_HCOUNTER, dwFormat PDH_FMT, rawValue1 *PDH_RAW_COUNTER, rawValue2 *PDH_RAW_COUNTER, fmtValue *PDH_FMT_COUNTERVALUE) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhCalculateCounterFromRawValue.Addr(), uintptr(hCounter), uintptr(dwFormat), uintptr(unsafe.Pointer(rawValue1)), uintptr(unsafe.Pointer(rawValue2)), uintptr(unsafe.Pointer(fmtValue)))
	return uint32(r1)
}

// PdhCloseLog calls pdh!PdhCloseLog.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhcloselog
// Minimum OS: windows5.1.2600.
func PdhCloseLog(hLog PDH_HLOG, dwFlags uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhCloseLog.Addr(), uintptr(hLog), uintptr(dwFlags))
	return uint32(r1)
}

// PdhCloseQuery calls pdh!PdhCloseQuery.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhclosequery
// Minimum OS: windows5.1.2600.
func PdhCloseQuery(hQuery PDH_HQUERY) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhCloseQuery.Addr(), uintptr(hQuery))
	return uint32(r1)
}

// PdhCollectQueryData calls pdh!PdhCollectQueryData.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhcollectquerydata
// Minimum OS: windows5.1.2600.
func PdhCollectQueryData(hQuery PDH_HQUERY) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhCollectQueryData.Addr(), uintptr(hQuery))
	return uint32(r1)
}

// PdhCollectQueryDataEx calls pdh!PdhCollectQueryDataEx.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhcollectquerydataex
// Minimum OS: windows5.1.2600.
func PdhCollectQueryDataEx(hQuery PDH_HQUERY, dwIntervalTime uint32, hNewDataEvent foundation.HANDLE) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhCollectQueryDataEx.Addr(), uintptr(hQuery), uintptr(dwIntervalTime), uintptr(hNewDataEvent))
	return uint32(r1)
}

// PdhCollectQueryDataWithTime calls pdh!PdhCollectQueryDataWithTime.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhcollectquerydatawithtime
// Minimum OS: windows6.0.6000.
func PdhCollectQueryDataWithTime(hQuery PDH_HQUERY, pllTimeStamp *int64) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhCollectQueryDataWithTime.Addr(), uintptr(hQuery), uintptr(unsafe.Pointer(pllTimeStamp)))
	return uint32(r1)
}

// PdhComputeCounterStatistics calls pdh!PdhComputeCounterStatistics.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhcomputecounterstatistics
// Minimum OS: windows5.1.2600.
func PdhComputeCounterStatistics(hCounter PDH_HCOUNTER, dwFormat PDH_FMT, dwFirstEntry uint32, dwNumEntries uint32, lpRawValueArray *PDH_RAW_COUNTER, data *PDH_STATISTICS) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhComputeCounterStatistics.Addr(), uintptr(hCounter), uintptr(dwFormat), uintptr(dwFirstEntry), uintptr(dwNumEntries), uintptr(unsafe.Pointer(lpRawValueArray)), uintptr(unsafe.Pointer(data)))
	return uint32(r1)
}

// PdhConnectMachine calls pdh!PdhConnectMachineW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhconnectmachinew
// Minimum OS: windows5.1.2600.
func PdhConnectMachine(szMachineName *string) uint32 {
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	r1, _, _ := syscall.SyscallN(procPdhConnectMachine.Addr(), uintptr(unsafe.Pointer(_szMachineName)))
	return uint32(r1)
}

// PdhConnectMachineA calls pdh!PdhConnectMachineA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhconnectmachinea
// Minimum OS: windows5.1.2600.
func PdhConnectMachineA(szMachineName foundation.PSTR) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhConnectMachineA.Addr(), uintptr(unsafe.Pointer(szMachineName)))
	return uint32(r1)
}

// PdhCreateSQLTables calls pdh!PdhCreateSQLTablesW.
func PdhCreateSQLTables(szDataSource string) uint32 {
	_szDataSource := win32.UTF16Ptr(szDataSource)
	r1, _, _ := syscall.SyscallN(procPdhCreateSQLTables.Addr(), uintptr(unsafe.Pointer(_szDataSource)))
	return uint32(r1)
}

// PdhCreateSQLTablesA calls pdh!PdhCreateSQLTablesA.
func PdhCreateSQLTablesA(szDataSource foundation.PSTR) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhCreateSQLTablesA.Addr(), uintptr(unsafe.Pointer(szDataSource)))
	return uint32(r1)
}

// PdhEnumLogSetNames calls pdh!PdhEnumLogSetNamesW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenumlogsetnamesw
// Minimum OS: windows5.1.2600.
func PdhEnumLogSetNames(szDataSource string, mszDataSetNameList foundation.PWSTR, pcchBufferLength *uint32) uint32 {
	_szDataSource := win32.UTF16Ptr(szDataSource)
	r1, _, _ := syscall.SyscallN(procPdhEnumLogSetNames.Addr(), uintptr(unsafe.Pointer(_szDataSource)), uintptr(unsafe.Pointer(mszDataSetNameList)), uintptr(unsafe.Pointer(pcchBufferLength)))
	return uint32(r1)
}

// PdhEnumLogSetNamesA calls pdh!PdhEnumLogSetNamesA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenumlogsetnamesa
// Minimum OS: windows5.1.2600.
func PdhEnumLogSetNamesA(szDataSource foundation.PSTR, mszDataSetNameList foundation.PSTR, pcchBufferLength *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhEnumLogSetNamesA.Addr(), uintptr(unsafe.Pointer(szDataSource)), uintptr(unsafe.Pointer(mszDataSetNameList)), uintptr(unsafe.Pointer(pcchBufferLength)))
	return uint32(r1)
}

// PdhEnumMachines calls pdh!PdhEnumMachinesW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenummachinesw
// Minimum OS: windows5.1.2600.
func PdhEnumMachines(szDataSource *string, mszMachineList foundation.PWSTR, pcchBufferSize *uint32) uint32 {
	_szDataSource := win32.UTF16PtrOrNil(szDataSource)
	r1, _, _ := syscall.SyscallN(procPdhEnumMachines.Addr(), uintptr(unsafe.Pointer(_szDataSource)), uintptr(unsafe.Pointer(mszMachineList)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhEnumMachinesA calls pdh!PdhEnumMachinesA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenummachinesa
// Minimum OS: windows5.1.2600.
func PdhEnumMachinesA(szDataSource foundation.PSTR, mszMachineList foundation.PSTR, pcchBufferSize *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhEnumMachinesA.Addr(), uintptr(unsafe.Pointer(szDataSource)), uintptr(unsafe.Pointer(mszMachineList)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhEnumMachinesH calls pdh!PdhEnumMachinesHW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenummachineshw
// Minimum OS: windows5.1.2600.
func PdhEnumMachinesH(hDataSource PDH_HLOG, mszMachineList foundation.PWSTR, pcchBufferSize *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhEnumMachinesH.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(mszMachineList)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhEnumMachinesHA calls pdh!PdhEnumMachinesHA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenummachinesha
// Minimum OS: windows5.1.2600.
func PdhEnumMachinesHA(hDataSource PDH_HLOG, mszMachineList foundation.PSTR, pcchBufferSize *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhEnumMachinesHA.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(mszMachineList)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhEnumObjectItems calls pdh!PdhEnumObjectItemsW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenumobjectitemsw
// Minimum OS: windows5.1.2600.
func PdhEnumObjectItems(szDataSource *string, szMachineName *string, szObjectName string, mszCounterList foundation.PWSTR, pcchCounterListLength *uint32, mszInstanceList foundation.PWSTR, pcchInstanceListLength *uint32, dwDetailLevel PERF_DETAIL, dwFlags uint32) uint32 {
	_szDataSource := win32.UTF16PtrOrNil(szDataSource)
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	_szObjectName := win32.UTF16Ptr(szObjectName)
	r1, _, _ := syscall.SyscallN(procPdhEnumObjectItems.Addr(), uintptr(unsafe.Pointer(_szDataSource)), uintptr(unsafe.Pointer(_szMachineName)), uintptr(unsafe.Pointer(_szObjectName)), uintptr(unsafe.Pointer(mszCounterList)), uintptr(unsafe.Pointer(pcchCounterListLength)), uintptr(unsafe.Pointer(mszInstanceList)), uintptr(unsafe.Pointer(pcchInstanceListLength)), uintptr(dwDetailLevel), uintptr(dwFlags))
	return uint32(r1)
}

// PdhEnumObjectItemsA calls pdh!PdhEnumObjectItemsA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenumobjectitemsa
// Minimum OS: windows5.1.2600.
func PdhEnumObjectItemsA(szDataSource foundation.PSTR, szMachineName foundation.PSTR, szObjectName foundation.PSTR, mszCounterList foundation.PSTR, pcchCounterListLength *uint32, mszInstanceList foundation.PSTR, pcchInstanceListLength *uint32, dwDetailLevel PERF_DETAIL, dwFlags uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhEnumObjectItemsA.Addr(), uintptr(unsafe.Pointer(szDataSource)), uintptr(unsafe.Pointer(szMachineName)), uintptr(unsafe.Pointer(szObjectName)), uintptr(unsafe.Pointer(mszCounterList)), uintptr(unsafe.Pointer(pcchCounterListLength)), uintptr(unsafe.Pointer(mszInstanceList)), uintptr(unsafe.Pointer(pcchInstanceListLength)), uintptr(dwDetailLevel), uintptr(dwFlags))
	return uint32(r1)
}

// PdhEnumObjectItemsH calls pdh!PdhEnumObjectItemsHW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenumobjectitemshw
// Minimum OS: windows5.1.2600.
func PdhEnumObjectItemsH(hDataSource PDH_HLOG, szMachineName *string, szObjectName string, mszCounterList foundation.PWSTR, pcchCounterListLength *uint32, mszInstanceList foundation.PWSTR, pcchInstanceListLength *uint32, dwDetailLevel PERF_DETAIL, dwFlags uint32) uint32 {
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	_szObjectName := win32.UTF16Ptr(szObjectName)
	r1, _, _ := syscall.SyscallN(procPdhEnumObjectItemsH.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(_szMachineName)), uintptr(unsafe.Pointer(_szObjectName)), uintptr(unsafe.Pointer(mszCounterList)), uintptr(unsafe.Pointer(pcchCounterListLength)), uintptr(unsafe.Pointer(mszInstanceList)), uintptr(unsafe.Pointer(pcchInstanceListLength)), uintptr(dwDetailLevel), uintptr(dwFlags))
	return uint32(r1)
}

// PdhEnumObjectItemsHA calls pdh!PdhEnumObjectItemsHA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenumobjectitemsha
// Minimum OS: windows5.1.2600.
func PdhEnumObjectItemsHA(hDataSource PDH_HLOG, szMachineName foundation.PSTR, szObjectName foundation.PSTR, mszCounterList foundation.PSTR, pcchCounterListLength *uint32, mszInstanceList foundation.PSTR, pcchInstanceListLength *uint32, dwDetailLevel PERF_DETAIL, dwFlags uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhEnumObjectItemsHA.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(szMachineName)), uintptr(unsafe.Pointer(szObjectName)), uintptr(unsafe.Pointer(mszCounterList)), uintptr(unsafe.Pointer(pcchCounterListLength)), uintptr(unsafe.Pointer(mszInstanceList)), uintptr(unsafe.Pointer(pcchInstanceListLength)), uintptr(dwDetailLevel), uintptr(dwFlags))
	return uint32(r1)
}

// PdhEnumObjects calls pdh!PdhEnumObjectsW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenumobjectsw
// Minimum OS: windows5.1.2600.
func PdhEnumObjects(szDataSource *string, szMachineName *string, mszObjectList foundation.PWSTR, pcchBufferSize *uint32, dwDetailLevel PERF_DETAIL, bRefresh bool) uint32 {
	_szDataSource := win32.UTF16PtrOrNil(szDataSource)
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	_bRefresh := win32.Bool32(bRefresh)
	r1, _, _ := syscall.SyscallN(procPdhEnumObjects.Addr(), uintptr(unsafe.Pointer(_szDataSource)), uintptr(unsafe.Pointer(_szMachineName)), uintptr(unsafe.Pointer(mszObjectList)), uintptr(unsafe.Pointer(pcchBufferSize)), uintptr(dwDetailLevel), uintptr(_bRefresh))
	return uint32(r1)
}

// PdhEnumObjectsA calls pdh!PdhEnumObjectsA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenumobjectsa
// Minimum OS: windows5.1.2600.
func PdhEnumObjectsA(szDataSource foundation.PSTR, szMachineName foundation.PSTR, mszObjectList foundation.PSTR, pcchBufferSize *uint32, dwDetailLevel PERF_DETAIL, bRefresh bool) uint32 {
	_bRefresh := win32.Bool32(bRefresh)
	r1, _, _ := syscall.SyscallN(procPdhEnumObjectsA.Addr(), uintptr(unsafe.Pointer(szDataSource)), uintptr(unsafe.Pointer(szMachineName)), uintptr(unsafe.Pointer(mszObjectList)), uintptr(unsafe.Pointer(pcchBufferSize)), uintptr(dwDetailLevel), uintptr(_bRefresh))
	return uint32(r1)
}

// PdhEnumObjectsH calls pdh!PdhEnumObjectsHW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenumobjectshw
// Minimum OS: windows5.1.2600.
func PdhEnumObjectsH(hDataSource PDH_HLOG, szMachineName *string, mszObjectList foundation.PWSTR, pcchBufferSize *uint32, dwDetailLevel PERF_DETAIL, bRefresh bool) uint32 {
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	_bRefresh := win32.Bool32(bRefresh)
	r1, _, _ := syscall.SyscallN(procPdhEnumObjectsH.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(_szMachineName)), uintptr(unsafe.Pointer(mszObjectList)), uintptr(unsafe.Pointer(pcchBufferSize)), uintptr(dwDetailLevel), uintptr(_bRefresh))
	return uint32(r1)
}

// PdhEnumObjectsHA calls pdh!PdhEnumObjectsHA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhenumobjectsha
// Minimum OS: windows5.1.2600.
func PdhEnumObjectsHA(hDataSource PDH_HLOG, szMachineName foundation.PSTR, mszObjectList foundation.PSTR, pcchBufferSize *uint32, dwDetailLevel PERF_DETAIL, bRefresh bool) uint32 {
	_bRefresh := win32.Bool32(bRefresh)
	r1, _, _ := syscall.SyscallN(procPdhEnumObjectsHA.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(szMachineName)), uintptr(unsafe.Pointer(mszObjectList)), uintptr(unsafe.Pointer(pcchBufferSize)), uintptr(dwDetailLevel), uintptr(_bRefresh))
	return uint32(r1)
}

// PdhExpandCounterPath calls pdh!PdhExpandCounterPathW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhexpandcounterpathw
// Minimum OS: windows5.1.2600.
func PdhExpandCounterPath(szWildCardPath string, mszExpandedPathList foundation.PWSTR, pcchPathListLength *uint32) uint32 {
	_szWildCardPath := win32.UTF16Ptr(szWildCardPath)
	r1, _, _ := syscall.SyscallN(procPdhExpandCounterPath.Addr(), uintptr(unsafe.Pointer(_szWildCardPath)), uintptr(unsafe.Pointer(mszExpandedPathList)), uintptr(unsafe.Pointer(pcchPathListLength)))
	return uint32(r1)
}

// PdhExpandCounterPathA calls pdh!PdhExpandCounterPathA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhexpandcounterpatha
// Minimum OS: windows5.1.2600.
func PdhExpandCounterPathA(szWildCardPath foundation.PSTR, mszExpandedPathList foundation.PSTR, pcchPathListLength *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhExpandCounterPathA.Addr(), uintptr(unsafe.Pointer(szWildCardPath)), uintptr(unsafe.Pointer(mszExpandedPathList)), uintptr(unsafe.Pointer(pcchPathListLength)))
	return uint32(r1)
}

// PdhExpandWildCardPath calls pdh!PdhExpandWildCardPathW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhexpandwildcardpathw
// Minimum OS: windows5.1.2600.
func PdhExpandWildCardPath(szDataSource *string, szWildCardPath string, mszExpandedPathList foundation.PWSTR, pcchPathListLength *uint32, dwFlags uint32) uint32 {
	_szDataSource := win32.UTF16PtrOrNil(szDataSource)
	_szWildCardPath := win32.UTF16Ptr(szWildCardPath)
	r1, _, _ := syscall.SyscallN(procPdhExpandWildCardPath.Addr(), uintptr(unsafe.Pointer(_szDataSource)), uintptr(unsafe.Pointer(_szWildCardPath)), uintptr(unsafe.Pointer(mszExpandedPathList)), uintptr(unsafe.Pointer(pcchPathListLength)), uintptr(dwFlags))
	return uint32(r1)
}

// PdhExpandWildCardPathA calls pdh!PdhExpandWildCardPathA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhexpandwildcardpatha
// Minimum OS: windows5.1.2600.
func PdhExpandWildCardPathA(szDataSource foundation.PSTR, szWildCardPath foundation.PSTR, mszExpandedPathList foundation.PSTR, pcchPathListLength *uint32, dwFlags uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhExpandWildCardPathA.Addr(), uintptr(unsafe.Pointer(szDataSource)), uintptr(unsafe.Pointer(szWildCardPath)), uintptr(unsafe.Pointer(mszExpandedPathList)), uintptr(unsafe.Pointer(pcchPathListLength)), uintptr(dwFlags))
	return uint32(r1)
}

// PdhExpandWildCardPathH calls pdh!PdhExpandWildCardPathHW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhexpandwildcardpathhw
// Minimum OS: windows5.1.2600.
func PdhExpandWildCardPathH(hDataSource PDH_HLOG, szWildCardPath string, mszExpandedPathList foundation.PWSTR, pcchPathListLength *uint32, dwFlags uint32) uint32 {
	_szWildCardPath := win32.UTF16Ptr(szWildCardPath)
	r1, _, _ := syscall.SyscallN(procPdhExpandWildCardPathH.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(_szWildCardPath)), uintptr(unsafe.Pointer(mszExpandedPathList)), uintptr(unsafe.Pointer(pcchPathListLength)), uintptr(dwFlags))
	return uint32(r1)
}

// PdhExpandWildCardPathHA calls pdh!PdhExpandWildCardPathHA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhexpandwildcardpathha
// Minimum OS: windows5.1.2600.
func PdhExpandWildCardPathHA(hDataSource PDH_HLOG, szWildCardPath foundation.PSTR, mszExpandedPathList foundation.PSTR, pcchPathListLength *uint32, dwFlags uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhExpandWildCardPathHA.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(szWildCardPath)), uintptr(unsafe.Pointer(mszExpandedPathList)), uintptr(unsafe.Pointer(pcchPathListLength)), uintptr(dwFlags))
	return uint32(r1)
}

// PdhFormatFromRawValue calls pdh!PdhFormatFromRawValue.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhformatfromrawvalue
// Minimum OS: windows5.1.2600.
func PdhFormatFromRawValue(dwCounterType uint32, dwFormat PDH_FMT, pTimeBase *int64, pRawValue1 *PDH_RAW_COUNTER, pRawValue2 *PDH_RAW_COUNTER, pFmtValue *PDH_FMT_COUNTERVALUE) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhFormatFromRawValue.Addr(), uintptr(dwCounterType), uintptr(dwFormat), uintptr(unsafe.Pointer(pTimeBase)), uintptr(unsafe.Pointer(pRawValue1)), uintptr(unsafe.Pointer(pRawValue2)), uintptr(unsafe.Pointer(pFmtValue)))
	return uint32(r1)
}

// PdhGetCounterInfo calls pdh!PdhGetCounterInfoW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetcounterinfow
// Minimum OS: windows5.1.2600.
func PdhGetCounterInfo(hCounter PDH_HCOUNTER, bRetrieveExplainText foundation.BOOLEAN, pdwBufferSize *uint32, lpBuffer *PDH_COUNTER_INFO_W) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetCounterInfo.Addr(), uintptr(hCounter), uintptr(bRetrieveExplainText), uintptr(unsafe.Pointer(pdwBufferSize)), uintptr(unsafe.Pointer(lpBuffer)))
	return uint32(r1)
}

// PdhGetCounterInfoA calls pdh!PdhGetCounterInfoA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetcounterinfoa
// Minimum OS: windows5.1.2600.
func PdhGetCounterInfoA(hCounter PDH_HCOUNTER, bRetrieveExplainText foundation.BOOLEAN, pdwBufferSize *uint32, lpBuffer *PDH_COUNTER_INFO_A) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetCounterInfoA.Addr(), uintptr(hCounter), uintptr(bRetrieveExplainText), uintptr(unsafe.Pointer(pdwBufferSize)), uintptr(unsafe.Pointer(lpBuffer)))
	return uint32(r1)
}

// PdhGetCounterTimeBase calls pdh!PdhGetCounterTimeBase.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetcountertimebase
// Minimum OS: windows5.1.2600.
func PdhGetCounterTimeBase(hCounter PDH_HCOUNTER, pTimeBase *int64) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetCounterTimeBase.Addr(), uintptr(hCounter), uintptr(unsafe.Pointer(pTimeBase)))
	return uint32(r1)
}

// PdhGetDataSourceTimeRange calls pdh!PdhGetDataSourceTimeRangeW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdatasourcetimerangew
// Minimum OS: windows5.1.2600.
func PdhGetDataSourceTimeRange(szDataSource *string, pdwNumEntries *uint32, pInfo *PDH_TIME_INFO, pdwBufferSize *uint32) uint32 {
	_szDataSource := win32.UTF16PtrOrNil(szDataSource)
	r1, _, _ := syscall.SyscallN(procPdhGetDataSourceTimeRange.Addr(), uintptr(unsafe.Pointer(_szDataSource)), uintptr(unsafe.Pointer(pdwNumEntries)), uintptr(unsafe.Pointer(pInfo)), uintptr(unsafe.Pointer(pdwBufferSize)))
	return uint32(r1)
}

// PdhGetDataSourceTimeRangeA calls pdh!PdhGetDataSourceTimeRangeA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdatasourcetimerangea
// Minimum OS: windows5.1.2600.
func PdhGetDataSourceTimeRangeA(szDataSource foundation.PSTR, pdwNumEntries *uint32, pInfo *PDH_TIME_INFO, pdwBufferSize *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetDataSourceTimeRangeA.Addr(), uintptr(unsafe.Pointer(szDataSource)), uintptr(unsafe.Pointer(pdwNumEntries)), uintptr(unsafe.Pointer(pInfo)), uintptr(unsafe.Pointer(pdwBufferSize)))
	return uint32(r1)
}

// PdhGetDataSourceTimeRangeH calls pdh!PdhGetDataSourceTimeRangeH.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdatasourcetimerangeh
// Minimum OS: windows5.1.2600.
func PdhGetDataSourceTimeRangeH(hDataSource PDH_HLOG, pdwNumEntries *uint32, pInfo *PDH_TIME_INFO, pdwBufferSize *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetDataSourceTimeRangeH.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(pdwNumEntries)), uintptr(unsafe.Pointer(pInfo)), uintptr(unsafe.Pointer(pdwBufferSize)))
	return uint32(r1)
}

// PdhGetDefaultPerfCounter calls pdh!PdhGetDefaultPerfCounterW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdefaultperfcounterw
// Minimum OS: windows5.1.2600.
func PdhGetDefaultPerfCounter(szDataSource *string, szMachineName *string, szObjectName string, szDefaultCounterName foundation.PWSTR, pcchBufferSize *uint32) uint32 {
	_szDataSource := win32.UTF16PtrOrNil(szDataSource)
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	_szObjectName := win32.UTF16Ptr(szObjectName)
	r1, _, _ := syscall.SyscallN(procPdhGetDefaultPerfCounter.Addr(), uintptr(unsafe.Pointer(_szDataSource)), uintptr(unsafe.Pointer(_szMachineName)), uintptr(unsafe.Pointer(_szObjectName)), uintptr(unsafe.Pointer(szDefaultCounterName)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhGetDefaultPerfCounterA calls pdh!PdhGetDefaultPerfCounterA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdefaultperfcountera
// Minimum OS: windows5.1.2600.
func PdhGetDefaultPerfCounterA(szDataSource foundation.PSTR, szMachineName foundation.PSTR, szObjectName foundation.PSTR, szDefaultCounterName foundation.PSTR, pcchBufferSize *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetDefaultPerfCounterA.Addr(), uintptr(unsafe.Pointer(szDataSource)), uintptr(unsafe.Pointer(szMachineName)), uintptr(unsafe.Pointer(szObjectName)), uintptr(unsafe.Pointer(szDefaultCounterName)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhGetDefaultPerfCounterH calls pdh!PdhGetDefaultPerfCounterHW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdefaultperfcounterhw
// Minimum OS: windows5.1.2600.
func PdhGetDefaultPerfCounterH(hDataSource PDH_HLOG, szMachineName *string, szObjectName string, szDefaultCounterName foundation.PWSTR, pcchBufferSize *uint32) uint32 {
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	_szObjectName := win32.UTF16Ptr(szObjectName)
	r1, _, _ := syscall.SyscallN(procPdhGetDefaultPerfCounterH.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(_szMachineName)), uintptr(unsafe.Pointer(_szObjectName)), uintptr(unsafe.Pointer(szDefaultCounterName)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhGetDefaultPerfCounterHA calls pdh!PdhGetDefaultPerfCounterHA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdefaultperfcounterha
// Minimum OS: windows5.1.2600.
func PdhGetDefaultPerfCounterHA(hDataSource PDH_HLOG, szMachineName foundation.PSTR, szObjectName foundation.PSTR, szDefaultCounterName foundation.PSTR, pcchBufferSize *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetDefaultPerfCounterHA.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(szMachineName)), uintptr(unsafe.Pointer(szObjectName)), uintptr(unsafe.Pointer(szDefaultCounterName)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhGetDefaultPerfObject calls pdh!PdhGetDefaultPerfObjectW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdefaultperfobjectw
// Minimum OS: windows5.1.2600.
func PdhGetDefaultPerfObject(szDataSource *string, szMachineName *string, szDefaultObjectName foundation.PWSTR, pcchBufferSize *uint32) uint32 {
	_szDataSource := win32.UTF16PtrOrNil(szDataSource)
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	r1, _, _ := syscall.SyscallN(procPdhGetDefaultPerfObject.Addr(), uintptr(unsafe.Pointer(_szDataSource)), uintptr(unsafe.Pointer(_szMachineName)), uintptr(unsafe.Pointer(szDefaultObjectName)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhGetDefaultPerfObjectA calls pdh!PdhGetDefaultPerfObjectA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdefaultperfobjecta
// Minimum OS: windows5.1.2600.
func PdhGetDefaultPerfObjectA(szDataSource foundation.PSTR, szMachineName foundation.PSTR, szDefaultObjectName foundation.PSTR, pcchBufferSize *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetDefaultPerfObjectA.Addr(), uintptr(unsafe.Pointer(szDataSource)), uintptr(unsafe.Pointer(szMachineName)), uintptr(unsafe.Pointer(szDefaultObjectName)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhGetDefaultPerfObjectH calls pdh!PdhGetDefaultPerfObjectHW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdefaultperfobjecthw
// Minimum OS: windows5.1.2600.
func PdhGetDefaultPerfObjectH(hDataSource PDH_HLOG, szMachineName *string, szDefaultObjectName foundation.PWSTR, pcchBufferSize *uint32) uint32 {
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	r1, _, _ := syscall.SyscallN(procPdhGetDefaultPerfObjectH.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(_szMachineName)), uintptr(unsafe.Pointer(szDefaultObjectName)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhGetDefaultPerfObjectHA calls pdh!PdhGetDefaultPerfObjectHA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdefaultperfobjectha
// Minimum OS: windows5.1.2600.
func PdhGetDefaultPerfObjectHA(hDataSource PDH_HLOG, szMachineName foundation.PSTR, szDefaultObjectName foundation.PSTR, pcchBufferSize *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetDefaultPerfObjectHA.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(szMachineName)), uintptr(unsafe.Pointer(szDefaultObjectName)), uintptr(unsafe.Pointer(pcchBufferSize)))
	return uint32(r1)
}

// PdhGetDllVersion calls pdh!PdhGetDllVersion.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetdllversion
// Minimum OS: windows5.1.2600.
func PdhGetDllVersion(lpdwVersion *PDH_DLL_VERSION) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetDllVersion.Addr(), uintptr(unsafe.Pointer(lpdwVersion)))
	return uint32(r1)
}

// PdhGetFormattedCounterArray calls pdh!PdhGetFormattedCounterArrayW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetformattedcounterarrayw
// Minimum OS: windows5.1.2600.
func PdhGetFormattedCounterArray(hCounter PDH_HCOUNTER, dwFormat PDH_FMT, lpdwBufferSize *uint32, lpdwItemCount *uint32, ItemBuffer *PDH_FMT_COUNTERVALUE_ITEM_W) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetFormattedCounterArray.Addr(), uintptr(hCounter), uintptr(dwFormat), uintptr(unsafe.Pointer(lpdwBufferSize)), uintptr(unsafe.Pointer(lpdwItemCount)), uintptr(unsafe.Pointer(ItemBuffer)))
	return uint32(r1)
}

// PdhGetFormattedCounterArrayA calls pdh!PdhGetFormattedCounterArrayA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetformattedcounterarraya
// Minimum OS: windows5.1.2600.
func PdhGetFormattedCounterArrayA(hCounter PDH_HCOUNTER, dwFormat PDH_FMT, lpdwBufferSize *uint32, lpdwItemCount *uint32, ItemBuffer *PDH_FMT_COUNTERVALUE_ITEM_A) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetFormattedCounterArrayA.Addr(), uintptr(hCounter), uintptr(dwFormat), uintptr(unsafe.Pointer(lpdwBufferSize)), uintptr(unsafe.Pointer(lpdwItemCount)), uintptr(unsafe.Pointer(ItemBuffer)))
	return uint32(r1)
}

// PdhGetFormattedCounterValue calls pdh!PdhGetFormattedCounterValue.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetformattedcountervalue
// Minimum OS: windows5.1.2600.
func PdhGetFormattedCounterValue(hCounter PDH_HCOUNTER, dwFormat PDH_FMT, lpdwType *uint32, pValue *PDH_FMT_COUNTERVALUE) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetFormattedCounterValue.Addr(), uintptr(hCounter), uintptr(dwFormat), uintptr(unsafe.Pointer(lpdwType)), uintptr(unsafe.Pointer(pValue)))
	return uint32(r1)
}

// PdhGetLogFileSize calls pdh!PdhGetLogFileSize.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetlogfilesize
// Minimum OS: windows5.1.2600.
func PdhGetLogFileSize(hLog PDH_HLOG, llSize *int64) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetLogFileSize.Addr(), uintptr(hLog), uintptr(unsafe.Pointer(llSize)))
	return uint32(r1)
}

// PdhGetLogSetGUID calls pdh!PdhGetLogSetGUID.
func PdhGetLogSetGUID(hLog PDH_HLOG, pGuid *win32.GUID, pRunId *int32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetLogSetGUID.Addr(), uintptr(hLog), uintptr(unsafe.Pointer(pGuid)), uintptr(unsafe.Pointer(pRunId)))
	return uint32(r1)
}

// PdhGetRawCounterArray calls pdh!PdhGetRawCounterArrayW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetrawcounterarrayw
// Minimum OS: windows5.1.2600.
func PdhGetRawCounterArray(hCounter PDH_HCOUNTER, lpdwBufferSize *uint32, lpdwItemCount *uint32, ItemBuffer *PDH_RAW_COUNTER_ITEM_W) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetRawCounterArray.Addr(), uintptr(hCounter), uintptr(unsafe.Pointer(lpdwBufferSize)), uintptr(unsafe.Pointer(lpdwItemCount)), uintptr(unsafe.Pointer(ItemBuffer)))
	return uint32(r1)
}

// PdhGetRawCounterArrayA calls pdh!PdhGetRawCounterArrayA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetrawcounterarraya
// Minimum OS: windows5.1.2600.
func PdhGetRawCounterArrayA(hCounter PDH_HCOUNTER, lpdwBufferSize *uint32, lpdwItemCount *uint32, ItemBuffer *PDH_RAW_COUNTER_ITEM_A) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetRawCounterArrayA.Addr(), uintptr(hCounter), uintptr(unsafe.Pointer(lpdwBufferSize)), uintptr(unsafe.Pointer(lpdwItemCount)), uintptr(unsafe.Pointer(ItemBuffer)))
	return uint32(r1)
}

// PdhGetRawCounterValue calls pdh!PdhGetRawCounterValue.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhgetrawcountervalue
// Minimum OS: windows5.1.2600.
func PdhGetRawCounterValue(hCounter PDH_HCOUNTER, lpdwType *uint32, pValue *PDH_RAW_COUNTER) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhGetRawCounterValue.Addr(), uintptr(hCounter), uintptr(unsafe.Pointer(lpdwType)), uintptr(unsafe.Pointer(pValue)))
	return uint32(r1)
}

// PdhIsRealTimeQuery calls pdh!PdhIsRealTimeQuery.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhisrealtimequery
// Minimum OS: windows5.1.2600.
func PdhIsRealTimeQuery(hQuery PDH_HQUERY) bool {
	r1, _, _ := syscall.SyscallN(procPdhIsRealTimeQuery.Addr(), uintptr(hQuery))
	return r1 != 0
}

// PdhLookupPerfIndexByName calls pdh!PdhLookupPerfIndexByNameW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhlookupperfindexbynamew
// Minimum OS: windows5.1.2600.
func PdhLookupPerfIndexByName(szMachineName *string, szNameBuffer string, pdwIndex *uint32) uint32 {
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	_szNameBuffer := win32.UTF16Ptr(szNameBuffer)
	r1, _, _ := syscall.SyscallN(procPdhLookupPerfIndexByName.Addr(), uintptr(unsafe.Pointer(_szMachineName)), uintptr(unsafe.Pointer(_szNameBuffer)), uintptr(unsafe.Pointer(pdwIndex)))
	return uint32(r1)
}

// PdhLookupPerfIndexByNameA calls pdh!PdhLookupPerfIndexByNameA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhlookupperfindexbynamea
// Minimum OS: windows5.1.2600.
func PdhLookupPerfIndexByNameA(szMachineName foundation.PSTR, szNameBuffer foundation.PSTR, pdwIndex *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhLookupPerfIndexByNameA.Addr(), uintptr(unsafe.Pointer(szMachineName)), uintptr(unsafe.Pointer(szNameBuffer)), uintptr(unsafe.Pointer(pdwIndex)))
	return uint32(r1)
}

// PdhLookupPerfNameByIndex calls pdh!PdhLookupPerfNameByIndexW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhlookupperfnamebyindexw
// Minimum OS: windows5.1.2600.
func PdhLookupPerfNameByIndex(szMachineName *string, dwNameIndex uint32, szNameBuffer foundation.PWSTR, pcchNameBufferSize *uint32) uint32 {
	_szMachineName := win32.UTF16PtrOrNil(szMachineName)
	r1, _, _ := syscall.SyscallN(procPdhLookupPerfNameByIndex.Addr(), uintptr(unsafe.Pointer(_szMachineName)), uintptr(dwNameIndex), uintptr(unsafe.Pointer(szNameBuffer)), uintptr(unsafe.Pointer(pcchNameBufferSize)))
	return uint32(r1)
}

// PdhLookupPerfNameByIndexA calls pdh!PdhLookupPerfNameByIndexA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhlookupperfnamebyindexa
// Minimum OS: windows5.1.2600.
func PdhLookupPerfNameByIndexA(szMachineName foundation.PSTR, dwNameIndex uint32, szNameBuffer foundation.PSTR, pcchNameBufferSize *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhLookupPerfNameByIndexA.Addr(), uintptr(unsafe.Pointer(szMachineName)), uintptr(dwNameIndex), uintptr(unsafe.Pointer(szNameBuffer)), uintptr(unsafe.Pointer(pcchNameBufferSize)))
	return uint32(r1)
}

// PdhMakeCounterPath calls pdh!PdhMakeCounterPathW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhmakecounterpathw
// Minimum OS: windows5.1.2600.
func PdhMakeCounterPath(pCounterPathElements *PDH_COUNTER_PATH_ELEMENTS_W, szFullPathBuffer foundation.PWSTR, pcchBufferSize *uint32, dwFlags PDH_PATH_FLAGS) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhMakeCounterPath.Addr(), uintptr(unsafe.Pointer(pCounterPathElements)), uintptr(unsafe.Pointer(szFullPathBuffer)), uintptr(unsafe.Pointer(pcchBufferSize)), uintptr(dwFlags))
	return uint32(r1)
}

// PdhMakeCounterPathA calls pdh!PdhMakeCounterPathA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhmakecounterpatha
// Minimum OS: windows5.1.2600.
func PdhMakeCounterPathA(pCounterPathElements *PDH_COUNTER_PATH_ELEMENTS_A, szFullPathBuffer foundation.PSTR, pcchBufferSize *uint32, dwFlags PDH_PATH_FLAGS) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhMakeCounterPathA.Addr(), uintptr(unsafe.Pointer(pCounterPathElements)), uintptr(unsafe.Pointer(szFullPathBuffer)), uintptr(unsafe.Pointer(pcchBufferSize)), uintptr(dwFlags))
	return uint32(r1)
}

// PdhOpenLog calls pdh!PdhOpenLogW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhopenlogw
// Minimum OS: windows5.1.2600.
func PdhOpenLog(szLogFileName string, dwAccessFlags PDH_LOG, lpdwLogType *PDH_LOG_TYPE, hQuery PDH_HQUERY, dwMaxSize uint32, szUserCaption *string, phLog *PDH_HLOG) uint32 {
	_szLogFileName := win32.UTF16Ptr(szLogFileName)
	_szUserCaption := win32.UTF16PtrOrNil(szUserCaption)
	r1, _, _ := syscall.SyscallN(procPdhOpenLog.Addr(), uintptr(unsafe.Pointer(_szLogFileName)), uintptr(dwAccessFlags), uintptr(unsafe.Pointer(lpdwLogType)), uintptr(hQuery), uintptr(dwMaxSize), uintptr(unsafe.Pointer(_szUserCaption)), uintptr(unsafe.Pointer(phLog)))
	return uint32(r1)
}

// PdhOpenLogA calls pdh!PdhOpenLogA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhopenloga
// Minimum OS: windows5.1.2600.
func PdhOpenLogA(szLogFileName foundation.PSTR, dwAccessFlags PDH_LOG, lpdwLogType *PDH_LOG_TYPE, hQuery PDH_HQUERY, dwMaxSize uint32, szUserCaption foundation.PSTR, phLog *PDH_HLOG) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhOpenLogA.Addr(), uintptr(unsafe.Pointer(szLogFileName)), uintptr(dwAccessFlags), uintptr(unsafe.Pointer(lpdwLogType)), uintptr(hQuery), uintptr(dwMaxSize), uintptr(unsafe.Pointer(szUserCaption)), uintptr(unsafe.Pointer(phLog)))
	return uint32(r1)
}

// PdhOpenQuery calls pdh!PdhOpenQueryW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhopenqueryw
// Minimum OS: windows5.1.2600.
func PdhOpenQuery(szDataSource *string, dwUserData uintptr, phQuery *PDH_HQUERY) uint32 {
	_szDataSource := win32.UTF16PtrOrNil(szDataSource)
	r1, _, _ := syscall.SyscallN(procPdhOpenQuery.Addr(), uintptr(unsafe.Pointer(_szDataSource)), uintptr(dwUserData), uintptr(unsafe.Pointer(phQuery)))
	return uint32(r1)
}

// PdhOpenQueryA calls pdh!PdhOpenQueryA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhopenquerya
// Minimum OS: windows5.1.2600.
func PdhOpenQueryA(szDataSource foundation.PSTR, dwUserData uintptr, phQuery *PDH_HQUERY) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhOpenQueryA.Addr(), uintptr(unsafe.Pointer(szDataSource)), uintptr(dwUserData), uintptr(unsafe.Pointer(phQuery)))
	return uint32(r1)
}

// PdhOpenQueryH calls pdh!PdhOpenQueryH.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhopenqueryh
// Minimum OS: windows5.1.2600.
func PdhOpenQueryH(hDataSource PDH_HLOG, dwUserData uintptr, phQuery *PDH_HQUERY) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhOpenQueryH.Addr(), uintptr(hDataSource), uintptr(dwUserData), uintptr(unsafe.Pointer(phQuery)))
	return uint32(r1)
}

// PdhParseCounterPath calls pdh!PdhParseCounterPathW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhparsecounterpathw
// Minimum OS: windows5.1.2600.
func PdhParseCounterPath(szFullPathBuffer string, pCounterPathElements *PDH_COUNTER_PATH_ELEMENTS_W, pdwBufferSize *uint32, dwFlags uint32) uint32 {
	_szFullPathBuffer := win32.UTF16Ptr(szFullPathBuffer)
	r1, _, _ := syscall.SyscallN(procPdhParseCounterPath.Addr(), uintptr(unsafe.Pointer(_szFullPathBuffer)), uintptr(unsafe.Pointer(pCounterPathElements)), uintptr(unsafe.Pointer(pdwBufferSize)), uintptr(dwFlags))
	return uint32(r1)
}

// PdhParseCounterPathA calls pdh!PdhParseCounterPathA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhparsecounterpatha
// Minimum OS: windows5.1.2600.
func PdhParseCounterPathA(szFullPathBuffer foundation.PSTR, pCounterPathElements *PDH_COUNTER_PATH_ELEMENTS_A, pdwBufferSize *uint32, dwFlags uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhParseCounterPathA.Addr(), uintptr(unsafe.Pointer(szFullPathBuffer)), uintptr(unsafe.Pointer(pCounterPathElements)), uintptr(unsafe.Pointer(pdwBufferSize)), uintptr(dwFlags))
	return uint32(r1)
}

// PdhParseInstanceName calls pdh!PdhParseInstanceNameW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhparseinstancenamew
// Minimum OS: windows5.1.2600.
func PdhParseInstanceName(szInstanceString string, szInstanceName foundation.PWSTR, pcchInstanceNameLength *uint32, szParentName foundation.PWSTR, pcchParentNameLength *uint32, lpIndex *uint32) uint32 {
	_szInstanceString := win32.UTF16Ptr(szInstanceString)
	r1, _, _ := syscall.SyscallN(procPdhParseInstanceName.Addr(), uintptr(unsafe.Pointer(_szInstanceString)), uintptr(unsafe.Pointer(szInstanceName)), uintptr(unsafe.Pointer(pcchInstanceNameLength)), uintptr(unsafe.Pointer(szParentName)), uintptr(unsafe.Pointer(pcchParentNameLength)), uintptr(unsafe.Pointer(lpIndex)))
	return uint32(r1)
}

// PdhParseInstanceNameA calls pdh!PdhParseInstanceNameA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhparseinstancenamea
// Minimum OS: windows5.1.2600.
func PdhParseInstanceNameA(szInstanceString foundation.PSTR, szInstanceName foundation.PSTR, pcchInstanceNameLength *uint32, szParentName foundation.PSTR, pcchParentNameLength *uint32, lpIndex *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhParseInstanceNameA.Addr(), uintptr(unsafe.Pointer(szInstanceString)), uintptr(unsafe.Pointer(szInstanceName)), uintptr(unsafe.Pointer(pcchInstanceNameLength)), uintptr(unsafe.Pointer(szParentName)), uintptr(unsafe.Pointer(pcchParentNameLength)), uintptr(unsafe.Pointer(lpIndex)))
	return uint32(r1)
}

// PdhReadRawLogRecord calls pdh!PdhReadRawLogRecord.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhreadrawlogrecord
// Minimum OS: windows5.1.2600.
func PdhReadRawLogRecord(hLog PDH_HLOG, ftRecord foundation.FILETIME, pRawLogRecord *PDH_RAW_LOG_RECORD, pdwBufferLength *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhReadRawLogRecord.Addr(), uintptr(hLog), uintptr(win32.StructArg(ftRecord)), uintptr(unsafe.Pointer(pRawLogRecord)), uintptr(unsafe.Pointer(pdwBufferLength)))
	return uint32(r1)
}

// PdhRemoveCounter calls pdh!PdhRemoveCounter.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhremovecounter
// Minimum OS: windows5.1.2600.
func PdhRemoveCounter(hCounter PDH_HCOUNTER) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhRemoveCounter.Addr(), uintptr(hCounter))
	return uint32(r1)
}

// PdhSelectDataSource calls pdh!PdhSelectDataSourceW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhselectdatasourcew
// Minimum OS: windows5.1.2600.
func PdhSelectDataSource(hWndOwner foundation.HWND, dwFlags PDH_SELECT_DATA_SOURCE_FLAGS, szDataSource foundation.PWSTR, pcchBufferLength *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhSelectDataSource.Addr(), uintptr(hWndOwner), uintptr(dwFlags), uintptr(unsafe.Pointer(szDataSource)), uintptr(unsafe.Pointer(pcchBufferLength)))
	return uint32(r1)
}

// PdhSelectDataSourceA calls pdh!PdhSelectDataSourceA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhselectdatasourcea
// Minimum OS: windows5.1.2600.
func PdhSelectDataSourceA(hWndOwner foundation.HWND, dwFlags PDH_SELECT_DATA_SOURCE_FLAGS, szDataSource foundation.PSTR, pcchBufferLength *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhSelectDataSourceA.Addr(), uintptr(hWndOwner), uintptr(dwFlags), uintptr(unsafe.Pointer(szDataSource)), uintptr(unsafe.Pointer(pcchBufferLength)))
	return uint32(r1)
}

// PdhSetCounterScaleFactor calls pdh!PdhSetCounterScaleFactor.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhsetcounterscalefactor
// Minimum OS: windows5.1.2600.
func PdhSetCounterScaleFactor(hCounter PDH_HCOUNTER, lFactor int32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhSetCounterScaleFactor.Addr(), uintptr(hCounter), uintptr(lFactor))
	return uint32(r1)
}

// PdhSetDefaultRealTimeDataSource calls pdh!PdhSetDefaultRealTimeDataSource.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhsetdefaultrealtimedatasource
// Minimum OS: windows5.1.2600.
func PdhSetDefaultRealTimeDataSource(dwDataSourceId REAL_TIME_DATA_SOURCE_ID_FLAGS) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhSetDefaultRealTimeDataSource.Addr(), uintptr(dwDataSourceId))
	return uint32(r1)
}

// PdhSetLogSetRunID calls pdh!PdhSetLogSetRunID.
func PdhSetLogSetRunID(hLog PDH_HLOG, RunId int32) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhSetLogSetRunID.Addr(), uintptr(hLog), uintptr(RunId))
	return uint32(r1)
}

// PdhSetQueryTimeRange calls pdh!PdhSetQueryTimeRange.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhsetquerytimerange
// Minimum OS: windows5.1.2600.
func PdhSetQueryTimeRange(hQuery PDH_HQUERY, pInfo *PDH_TIME_INFO) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhSetQueryTimeRange.Addr(), uintptr(hQuery), uintptr(unsafe.Pointer(pInfo)))
	return uint32(r1)
}

// PdhUpdateLog calls pdh!PdhUpdateLogW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhupdatelogw
// Minimum OS: windows5.1.2600.
func PdhUpdateLog(hLog PDH_HLOG, szUserString *string) uint32 {
	_szUserString := win32.UTF16PtrOrNil(szUserString)
	r1, _, _ := syscall.SyscallN(procPdhUpdateLog.Addr(), uintptr(hLog), uintptr(unsafe.Pointer(_szUserString)))
	return uint32(r1)
}

// PdhUpdateLogA calls pdh!PdhUpdateLogA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhupdateloga
// Minimum OS: windows5.1.2600.
func PdhUpdateLogA(hLog PDH_HLOG, szUserString foundation.PSTR) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhUpdateLogA.Addr(), uintptr(hLog), uintptr(unsafe.Pointer(szUserString)))
	return uint32(r1)
}

// PdhUpdateLogFileCatalog calls pdh!PdhUpdateLogFileCatalog.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhupdatelogfilecatalog
// Minimum OS: windows5.1.2600.
func PdhUpdateLogFileCatalog(hLog PDH_HLOG) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhUpdateLogFileCatalog.Addr(), uintptr(hLog))
	return uint32(r1)
}

// PdhValidatePath calls pdh!PdhValidatePathW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhvalidatepathw
// Minimum OS: windows5.1.2600.
func PdhValidatePath(szFullPathBuffer string) uint32 {
	_szFullPathBuffer := win32.UTF16Ptr(szFullPathBuffer)
	r1, _, _ := syscall.SyscallN(procPdhValidatePath.Addr(), uintptr(unsafe.Pointer(_szFullPathBuffer)))
	return uint32(r1)
}

// PdhValidatePathA calls pdh!PdhValidatePathA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhvalidatepatha
// Minimum OS: windows5.1.2600.
func PdhValidatePathA(szFullPathBuffer foundation.PSTR) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhValidatePathA.Addr(), uintptr(unsafe.Pointer(szFullPathBuffer)))
	return uint32(r1)
}

// PdhValidatePathEx calls pdh!PdhValidatePathExW.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhvalidatepathexw
// Minimum OS: windows6.0.6000.
func PdhValidatePathEx(hDataSource PDH_HLOG, szFullPathBuffer string) uint32 {
	_szFullPathBuffer := win32.UTF16Ptr(szFullPathBuffer)
	r1, _, _ := syscall.SyscallN(procPdhValidatePathEx.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(_szFullPathBuffer)))
	return uint32(r1)
}

// PdhValidatePathExA calls pdh!PdhValidatePathExA.
// https://learn.microsoft.com/windows/win32/api/pdh/nf-pdh-pdhvalidatepathexa
// Minimum OS: windows6.0.6000.
func PdhValidatePathExA(hDataSource PDH_HLOG, szFullPathBuffer foundation.PSTR) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhValidatePathExA.Addr(), uintptr(hDataSource), uintptr(unsafe.Pointer(szFullPathBuffer)))
	return uint32(r1)
}

// PdhVerifySQLDB calls pdh!PdhVerifySQLDBW.
func PdhVerifySQLDB(szDataSource string) uint32 {
	_szDataSource := win32.UTF16Ptr(szDataSource)
	r1, _, _ := syscall.SyscallN(procPdhVerifySQLDB.Addr(), uintptr(unsafe.Pointer(_szDataSource)))
	return uint32(r1)
}

// PdhVerifySQLDBA calls pdh!PdhVerifySQLDBA.
func PdhVerifySQLDBA(szDataSource foundation.PSTR) uint32 {
	r1, _, _ := syscall.SyscallN(procPdhVerifySQLDBA.Addr(), uintptr(unsafe.Pointer(szDataSource)))
	return uint32(r1)
}

// PerfAddCounters calls ADVAPI32!PerfAddCounters.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfaddcounters
// Minimum OS: windows10.0.14393.
func PerfAddCounters(hQuery foundation.HANDLE, pCounters *PERF_COUNTER_IDENTIFIER, cbCounters uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfAddCounters.Addr(), uintptr(hQuery), uintptr(unsafe.Pointer(pCounters)), uintptr(cbCounters))
	return uint32(r1)
}

// PerfCloseQueryHandle calls ADVAPI32!PerfCloseQueryHandle.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfclosequeryhandle
// Minimum OS: windows10.0.14393.
func PerfCloseQueryHandle(hQuery foundation.HANDLE) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfCloseQueryHandle.Addr(), uintptr(hQuery))
	return uint32(r1)
}

// PerfCreateInstance calls ADVAPI32!PerfCreateInstance.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfcreateinstance
// Minimum OS: windows6.0.6000.
func PerfCreateInstance(ProviderHandle foundation.HANDLE, CounterSetGuid *win32.GUID, Name string, Id uint32) (*PERF_COUNTERSET_INSTANCE, error) {
	_Name := win32.UTF16Ptr(Name)
	r1, _, e1 := syscall.SyscallN(procPerfCreateInstance.Addr(), uintptr(ProviderHandle), uintptr(unsafe.Pointer(CounterSetGuid)), uintptr(unsafe.Pointer(_Name)), uintptr(Id))
	ret := (*PERF_COUNTERSET_INSTANCE)(unsafe.Pointer(r1))
	if ret == nil {
		return ret, win32.LastError(e1)
	}
	return ret, nil
}

// PerfDecrementULongCounterValue calls ADVAPI32!PerfDecrementULongCounterValue.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfdecrementulongcountervalue
// Minimum OS: windows6.0.6000.
func PerfDecrementULongCounterValue(Provider foundation.HANDLE, Instance *PERF_COUNTERSET_INSTANCE, CounterId uint32, Value uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfDecrementULongCounterValue.Addr(), uintptr(Provider), uintptr(unsafe.Pointer(Instance)), uintptr(CounterId), uintptr(Value))
	return uint32(r1)
}

// PerfDecrementULongLongCounterValue calls ADVAPI32!PerfDecrementULongLongCounterValue.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfdecrementulonglongcountervalue
// Minimum OS: windows6.0.6000.
func PerfDecrementULongLongCounterValue(Provider foundation.HANDLE, Instance *PERF_COUNTERSET_INSTANCE, CounterId uint32, Value uint64) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfDecrementULongLongCounterValue.Addr(), uintptr(Provider), uintptr(unsafe.Pointer(Instance)), uintptr(CounterId), uintptr(Value))
	return uint32(r1)
}

// PerfDeleteCounters calls ADVAPI32!PerfDeleteCounters.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfdeletecounters
// Minimum OS: windows10.0.14393.
func PerfDeleteCounters(hQuery foundation.HANDLE, pCounters *PERF_COUNTER_IDENTIFIER, cbCounters uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfDeleteCounters.Addr(), uintptr(hQuery), uintptr(unsafe.Pointer(pCounters)), uintptr(cbCounters))
	return uint32(r1)
}

// PerfDeleteInstance calls ADVAPI32!PerfDeleteInstance.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfdeleteinstance
// Minimum OS: windows6.0.6000.
func PerfDeleteInstance(Provider foundation.HANDLE, InstanceBlock *PERF_COUNTERSET_INSTANCE) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfDeleteInstance.Addr(), uintptr(Provider), uintptr(unsafe.Pointer(InstanceBlock)))
	return uint32(r1)
}

// PerfEnumerateCounterSet calls ADVAPI32!PerfEnumerateCounterSet.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfenumeratecounterset
// Minimum OS: windows10.0.14393.
func PerfEnumerateCounterSet(szMachine *string, pCounterSetIds []win32.GUID, pcCounterSetIdsActual *uint32) uint32 {
	_szMachine := win32.UTF16PtrOrNil(szMachine)
	var _pCounterSetIds *win32.GUID
	if len(pCounterSetIds) > 0 {
		_pCounterSetIds = &pCounterSetIds[0]
	}
	r1, _, _ := syscall.SyscallN(procPerfEnumerateCounterSet.Addr(), uintptr(unsafe.Pointer(_szMachine)), uintptr(unsafe.Pointer(_pCounterSetIds)), uintptr(len(pCounterSetIds)), uintptr(unsafe.Pointer(pcCounterSetIdsActual)))
	return uint32(r1)
}

// PerfEnumerateCounterSetInstances calls ADVAPI32!PerfEnumerateCounterSetInstances.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfenumeratecountersetinstances
// Minimum OS: windows10.0.14393.
func PerfEnumerateCounterSetInstances(szMachine *string, pCounterSetId *win32.GUID, pInstances *PERF_INSTANCE_HEADER, cbInstances uint32, pcbInstancesActual *uint32) uint32 {
	_szMachine := win32.UTF16PtrOrNil(szMachine)
	r1, _, _ := syscall.SyscallN(procPerfEnumerateCounterSetInstances.Addr(), uintptr(unsafe.Pointer(_szMachine)), uintptr(unsafe.Pointer(pCounterSetId)), uintptr(unsafe.Pointer(pInstances)), uintptr(cbInstances), uintptr(unsafe.Pointer(pcbInstancesActual)))
	return uint32(r1)
}

// PerfIncrementULongCounterValue calls ADVAPI32!PerfIncrementULongCounterValue.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfincrementulongcountervalue
// Minimum OS: windows6.0.6000.
func PerfIncrementULongCounterValue(Provider foundation.HANDLE, Instance *PERF_COUNTERSET_INSTANCE, CounterId uint32, Value uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfIncrementULongCounterValue.Addr(), uintptr(Provider), uintptr(unsafe.Pointer(Instance)), uintptr(CounterId), uintptr(Value))
	return uint32(r1)
}

// PerfIncrementULongLongCounterValue calls ADVAPI32!PerfIncrementULongLongCounterValue.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfincrementulonglongcountervalue
// Minimum OS: windows6.0.6000.
func PerfIncrementULongLongCounterValue(Provider foundation.HANDLE, Instance *PERF_COUNTERSET_INSTANCE, CounterId uint32, Value uint64) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfIncrementULongLongCounterValue.Addr(), uintptr(Provider), uintptr(unsafe.Pointer(Instance)), uintptr(CounterId), uintptr(Value))
	return uint32(r1)
}

// PerfOpenQueryHandle calls ADVAPI32!PerfOpenQueryHandle.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfopenqueryhandle
// Minimum OS: windows10.0.14393.
func PerfOpenQueryHandle(szMachine *string, phQuery *foundation.HANDLE) uint32 {
	_szMachine := win32.UTF16PtrOrNil(szMachine)
	r1, _, _ := syscall.SyscallN(procPerfOpenQueryHandle.Addr(), uintptr(unsafe.Pointer(_szMachine)), uintptr(unsafe.Pointer(phQuery)))
	return uint32(r1)
}

// PerfQueryCounterData calls ADVAPI32!PerfQueryCounterData.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfquerycounterdata
// Minimum OS: windows10.0.14393.
func PerfQueryCounterData(hQuery foundation.HANDLE, pCounterBlock *PERF_DATA_HEADER, cbCounterBlock uint32, pcbCounterBlockActual *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfQueryCounterData.Addr(), uintptr(hQuery), uintptr(unsafe.Pointer(pCounterBlock)), uintptr(cbCounterBlock), uintptr(unsafe.Pointer(pcbCounterBlockActual)))
	return uint32(r1)
}

// PerfQueryCounterInfo calls ADVAPI32!PerfQueryCounterInfo.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfquerycounterinfo
// Minimum OS: windows10.0.14393.
func PerfQueryCounterInfo(hQuery foundation.HANDLE, pCounters *PERF_COUNTER_IDENTIFIER, cbCounters uint32, pcbCountersActual *uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfQueryCounterInfo.Addr(), uintptr(hQuery), uintptr(unsafe.Pointer(pCounters)), uintptr(cbCounters), uintptr(unsafe.Pointer(pcbCountersActual)))
	return uint32(r1)
}

// PerfQueryCounterSetRegistrationInfo calls ADVAPI32!PerfQueryCounterSetRegistrationInfo.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfquerycountersetregistrationinfo
// Minimum OS: windows10.0.14393.
func PerfQueryCounterSetRegistrationInfo(szMachine *string, pCounterSetId *win32.GUID, requestCode PerfRegInfoType, requestLangId uint32, pbRegInfo []byte, pcbRegInfoActual *uint32) uint32 {
	_szMachine := win32.UTF16PtrOrNil(szMachine)
	var _pbRegInfo *byte
	if len(pbRegInfo) > 0 {
		_pbRegInfo = &pbRegInfo[0]
	}
	r1, _, _ := syscall.SyscallN(procPerfQueryCounterSetRegistrationInfo.Addr(), uintptr(unsafe.Pointer(_szMachine)), uintptr(unsafe.Pointer(pCounterSetId)), uintptr(requestCode), uintptr(requestLangId), uintptr(unsafe.Pointer(_pbRegInfo)), uintptr(len(pbRegInfo)), uintptr(unsafe.Pointer(pcbRegInfoActual)))
	return uint32(r1)
}

// PerfQueryInstance calls ADVAPI32!PerfQueryInstance.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfqueryinstance
// Minimum OS: windows6.0.6000.
func PerfQueryInstance(ProviderHandle foundation.HANDLE, CounterSetGuid *win32.GUID, Name string, Id uint32) (*PERF_COUNTERSET_INSTANCE, error) {
	_Name := win32.UTF16Ptr(Name)
	r1, _, e1 := syscall.SyscallN(procPerfQueryInstance.Addr(), uintptr(ProviderHandle), uintptr(unsafe.Pointer(CounterSetGuid)), uintptr(unsafe.Pointer(_Name)), uintptr(Id))
	ret := (*PERF_COUNTERSET_INSTANCE)(unsafe.Pointer(r1))
	if ret == nil {
		return ret, win32.LastError(e1)
	}
	return ret, nil
}

// PerfSetCounterRefValue calls ADVAPI32!PerfSetCounterRefValue.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfsetcounterrefvalue
// Minimum OS: windows6.0.6000.
func PerfSetCounterRefValue(Provider foundation.HANDLE, Instance *PERF_COUNTERSET_INSTANCE, CounterId uint32, Address unsafe.Pointer) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfSetCounterRefValue.Addr(), uintptr(Provider), uintptr(unsafe.Pointer(Instance)), uintptr(CounterId), uintptr(unsafe.Pointer(Address)))
	return uint32(r1)
}

// PerfSetCounterSetInfo calls ADVAPI32!PerfSetCounterSetInfo.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfsetcountersetinfo
// Minimum OS: windows6.0.6000.
func PerfSetCounterSetInfo(ProviderHandle foundation.HANDLE, Template *PERF_COUNTERSET_INFO, TemplateSize uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfSetCounterSetInfo.Addr(), uintptr(ProviderHandle), uintptr(unsafe.Pointer(Template)), uintptr(TemplateSize))
	return uint32(r1)
}

// PerfSetULongCounterValue calls ADVAPI32!PerfSetULongCounterValue.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfsetulongcountervalue
// Minimum OS: windows6.0.6000.
func PerfSetULongCounterValue(Provider foundation.HANDLE, Instance *PERF_COUNTERSET_INSTANCE, CounterId uint32, Value uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfSetULongCounterValue.Addr(), uintptr(Provider), uintptr(unsafe.Pointer(Instance)), uintptr(CounterId), uintptr(Value))
	return uint32(r1)
}

// PerfSetULongLongCounterValue calls ADVAPI32!PerfSetULongLongCounterValue.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfsetulonglongcountervalue
// Minimum OS: windows6.0.6000.
func PerfSetULongLongCounterValue(Provider foundation.HANDLE, Instance *PERF_COUNTERSET_INSTANCE, CounterId uint32, Value uint64) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfSetULongLongCounterValue.Addr(), uintptr(Provider), uintptr(unsafe.Pointer(Instance)), uintptr(CounterId), uintptr(Value))
	return uint32(r1)
}

// PerfStartProvider calls ADVAPI32!PerfStartProvider.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfstartprovider
// Minimum OS: windows6.0.6000.
func PerfStartProvider(ProviderGuid *win32.GUID, ControlCallback PERFLIBREQUEST, phProvider *foundation.HANDLE) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfStartProvider.Addr(), uintptr(unsafe.Pointer(ProviderGuid)), uintptr(ControlCallback), uintptr(unsafe.Pointer(phProvider)))
	return uint32(r1)
}

// PerfStartProviderEx calls ADVAPI32!PerfStartProviderEx.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfstartproviderex
// Minimum OS: windows6.0.6000.
func PerfStartProviderEx(ProviderGuid *win32.GUID, ProviderContext *PERF_PROVIDER_CONTEXT, Provider *foundation.HANDLE) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfStartProviderEx.Addr(), uintptr(unsafe.Pointer(ProviderGuid)), uintptr(unsafe.Pointer(ProviderContext)), uintptr(unsafe.Pointer(Provider)))
	return uint32(r1)
}

// PerfStopProvider calls ADVAPI32!PerfStopProvider.
// https://learn.microsoft.com/windows/win32/api/perflib/nf-perflib-perfstopprovider
// Minimum OS: windows6.0.6000.
func PerfStopProvider(ProviderHandle foundation.HANDLE) uint32 {
	r1, _, _ := syscall.SyscallN(procPerfStopProvider.Addr(), uintptr(ProviderHandle))
	return uint32(r1)
}

// QueryPerformanceCounter calls KERNEL32!QueryPerformanceCounter.
// https://learn.microsoft.com/windows/win32/api/profileapi/nf-profileapi-queryperformancecounter
// Minimum OS: windows5.0.
func QueryPerformanceCounter(lpPerformanceCount *int64) error {
	r1, _, e1 := syscall.SyscallN(procQueryPerformanceCounter.Addr(), uintptr(unsafe.Pointer(lpPerformanceCount)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// QueryPerformanceFrequency calls KERNEL32!QueryPerformanceFrequency.
// https://learn.microsoft.com/windows/win32/api/profileapi/nf-profileapi-queryperformancefrequency
// Minimum OS: windows5.0.
func QueryPerformanceFrequency(lpFrequency *int64) error {
	r1, _, e1 := syscall.SyscallN(procQueryPerformanceFrequency.Addr(), uintptr(unsafe.Pointer(lpFrequency)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// RestorePerfRegistryFromFileW calls loadperf!RestorePerfRegistryFromFileW.
func RestorePerfRegistryFromFileW(szFileName *string, szLangId *string) uint32 {
	_szFileName := win32.UTF16PtrOrNil(szFileName)
	_szLangId := win32.UTF16PtrOrNil(szLangId)
	r1, _, _ := syscall.SyscallN(procRestorePerfRegistryFromFileW.Addr(), uintptr(unsafe.Pointer(_szFileName)), uintptr(unsafe.Pointer(_szLangId)))
	return uint32(r1)
}

// SetServiceAsTrusted calls loadperf!SetServiceAsTrustedW.
func SetServiceAsTrusted(szReserved *string, szServiceName string) uint32 {
	_szReserved := win32.UTF16PtrOrNil(szReserved)
	_szServiceName := win32.UTF16Ptr(szServiceName)
	r1, _, _ := syscall.SyscallN(procSetServiceAsTrusted.Addr(), uintptr(unsafe.Pointer(_szReserved)), uintptr(unsafe.Pointer(_szServiceName)))
	return uint32(r1)
}

// SetServiceAsTrustedA calls loadperf!SetServiceAsTrustedA.
func SetServiceAsTrustedA(szReserved foundation.PSTR, szServiceName foundation.PSTR) uint32 {
	r1, _, _ := syscall.SyscallN(procSetServiceAsTrustedA.Addr(), uintptr(unsafe.Pointer(szReserved)), uintptr(unsafe.Pointer(szServiceName)))
	return uint32(r1)
}

// UnloadPerfCounterTextStrings calls loadperf!UnloadPerfCounterTextStringsW.
// https://learn.microsoft.com/windows/win32/api/loadperf/nf-loadperf-unloadperfcountertextstringsw
// Minimum OS: windows5.1.2600.
func UnloadPerfCounterTextStrings(lpCommandLine string, bQuietModeArg bool) uint32 {
	_lpCommandLine := win32.UTF16Ptr(lpCommandLine)
	_bQuietModeArg := win32.Bool32(bQuietModeArg)
	r1, _, _ := syscall.SyscallN(procUnloadPerfCounterTextStrings.Addr(), uintptr(unsafe.Pointer(_lpCommandLine)), uintptr(_bQuietModeArg))
	return uint32(r1)
}

// UnloadPerfCounterTextStringsA calls loadperf!UnloadPerfCounterTextStringsA.
// https://learn.microsoft.com/windows/win32/api/loadperf/nf-loadperf-unloadperfcountertextstringsa
// Minimum OS: windows5.1.2600.
func UnloadPerfCounterTextStringsA(lpCommandLine foundation.PSTR, bQuietModeArg bool) uint32 {
	_bQuietModeArg := win32.Bool32(bQuietModeArg)
	r1, _, _ := syscall.SyscallN(procUnloadPerfCounterTextStringsA.Addr(), uintptr(unsafe.Pointer(lpCommandLine)), uintptr(_bQuietModeArg))
	return uint32(r1)
}

// UpdatePerfNameFiles calls loadperf!UpdatePerfNameFilesW.
func UpdatePerfNameFiles(szNewCtrFilePath string, szNewHlpFilePath *string, szLanguageID string, dwModes uintptr) uint32 {
	_szNewCtrFilePath := win32.UTF16Ptr(szNewCtrFilePath)
	_szNewHlpFilePath := win32.UTF16PtrOrNil(szNewHlpFilePath)
	_szLanguageID := win32.UTF16Ptr(szLanguageID)
	r1, _, _ := syscall.SyscallN(procUpdatePerfNameFiles.Addr(), uintptr(unsafe.Pointer(_szNewCtrFilePath)), uintptr(unsafe.Pointer(_szNewHlpFilePath)), uintptr(unsafe.Pointer(_szLanguageID)), uintptr(dwModes))
	return uint32(r1)
}

// UpdatePerfNameFilesA calls loadperf!UpdatePerfNameFilesA.
func UpdatePerfNameFilesA(szNewCtrFilePath foundation.PSTR, szNewHlpFilePath foundation.PSTR, szLanguageID foundation.PSTR, dwModes uintptr) uint32 {
	r1, _, _ := syscall.SyscallN(procUpdatePerfNameFilesA.Addr(), uintptr(unsafe.Pointer(szNewCtrFilePath)), uintptr(unsafe.Pointer(szNewHlpFilePath)), uintptr(unsafe.Pointer(szLanguageID)), uintptr(dwModes))
	return uint32(r1)
}
