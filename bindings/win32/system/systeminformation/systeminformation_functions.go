// Code generated by go-bindings-win32-codegen. DO NOT EDIT.

//go:build windows && (amd64 || arm64)

package systeminformation

import (
	"syscall"
	"unsafe"

	"github.com/deploymenttheory/go-bindings-win32/bindings/runtime/win32"
	"github.com/deploymenttheory/go-bindings-win32/bindings/win32/foundation"
)

var (
	modapi_ms_win_core_sysinfo_l1_2_0 = win32.NewDLL("api-ms-win-core-sysinfo-l1-2-0.dll")
	modapi_ms_win_core_sysinfo_l1_2_3 = win32.NewDLL("api-ms-win-core-sysinfo-l1-2-3.dll")
	modapi_ms_win_core_sysinfo_l1_2_4 = win32.NewDLL("api-ms-win-core-sysinfo-l1-2-4.dll")
	modapi_ms_win_core_sysinfo_l1_2_6 = win32.NewDLL("api-ms-win-core-sysinfo-l1-2-6.dll")
	modapi_ms_win_core_wow64_l1_1_1   = win32.NewDLL("api-ms-win-core-wow64-l1-1-1.dll")
	modKERNEL32                       = win32.NewDLL("KERNEL32.dll")
	modntdll                          = win32.NewDLL("ntdll.dll")
	modntdllk                         = win32.NewDLL("ntdllk.dll")
)

var (
	procGetOsSafeBootMode                  = modapi_ms_win_core_sysinfo_l1_2_0.NewProc("GetOsSafeBootMode")
	procGetIntegratedDisplaySize           = modapi_ms_win_core_sysinfo_l1_2_3.NewProc("GetIntegratedDisplaySize")
	procGetOsManufacturingMode             = modapi_ms_win_core_sysinfo_l1_2_3.NewProc("GetOsManufacturingMode")
	procGetSystemTimeAdjustmentPrecise     = modapi_ms_win_core_sysinfo_l1_2_4.NewProc("GetSystemTimeAdjustmentPrecise")
	procSetSystemTimeAdjustmentPrecise     = modapi_ms_win_core_sysinfo_l1_2_4.NewProc("SetSystemTimeAdjustmentPrecise")
	procGetDeveloperDriveEnablementState   = modapi_ms_win_core_sysinfo_l1_2_6.NewProc("GetDeveloperDriveEnablementState")
	procGetSystemWow64Directory2           = modapi_ms_win_core_wow64_l1_1_1.NewProc("GetSystemWow64Directory2W")
	procGetSystemWow64Directory2A          = modapi_ms_win_core_wow64_l1_1_1.NewProc("GetSystemWow64Directory2A")
	procDnsHostnameToComputerNameExW       = modKERNEL32.NewProc("DnsHostnameToComputerNameExW")
	procEnumSystemFirmwareTables           = modKERNEL32.NewProc("EnumSystemFirmwareTables")
	procGetComputerNameEx                  = modKERNEL32.NewProc("GetComputerNameExW")
	procGetComputerNameExA                 = modKERNEL32.NewProc("GetComputerNameExA")
	procGetFirmwareType                    = modKERNEL32.NewProc("GetFirmwareType")
	procGetLocalTime                       = modKERNEL32.NewProc("GetLocalTime")
	procGetLogicalProcessorInformation     = modKERNEL32.NewProc("GetLogicalProcessorInformation")
	procGetLogicalProcessorInformationEx   = modKERNEL32.NewProc("GetLogicalProcessorInformationEx")
	procGetNativeSystemInfo                = modKERNEL32.NewProc("GetNativeSystemInfo")
	procGetPhysicallyInstalledSystemMemory = modKERNEL32.NewProc("GetPhysicallyInstalledSystemMemory")
	procGetProcessorSystemCycleTime        = modKERNEL32.NewProc("GetProcessorSystemCycleTime")
	procGetProductInfo                     = modKERNEL32.NewProc("GetProductInfo")
	procGetRuntimeAttestationReport        = modKERNEL32.NewProc("GetRuntimeAttestationReport")
	procGetSystemCpuSetInformation         = modKERNEL32.NewProc("GetSystemCpuSetInformation")
	procGetSystemDEPPolicy                 = modKERNEL32.NewProc("GetSystemDEPPolicy")
	procGetSystemDirectory                 = modKERNEL32.NewProc("GetSystemDirectoryW")
	procGetSystemDirectoryA                = modKERNEL32.NewProc("GetSystemDirectoryA")
	procGetSystemFirmwareTable             = modKERNEL32.NewProc("GetSystemFirmwareTable")
	procGetSystemInfo                      = modKERNEL32.NewProc("GetSystemInfo")
	procGetSystemLeapSecondInformation     = modKERNEL32.NewProc("GetSystemLeapSecondInformation")
	procGetSystemTime                      = modKERNEL32.NewProc("GetSystemTime")
	procGetSystemTimeAdjustment            = modKERNEL32.NewProc("GetSystemTimeAdjustment")
	procGetSystemTimeAsFileTime            = modKERNEL32.NewProc("GetSystemTimeAsFileTime")
	procGetSystemTimePreciseAsFileTime     = modKERNEL32.NewProc("GetSystemTimePreciseAsFileTime")
	procGetSystemWindowsDirectory          = modKERNEL32.NewProc("GetSystemWindowsDirectoryW")
	procGetSystemWindowsDirectoryA         = modKERNEL32.NewProc("GetSystemWindowsDirectoryA")
	procGetSystemWow64Directory            = modKERNEL32.NewProc("GetSystemWow64DirectoryW")
	procGetSystemWow64DirectoryA           = modKERNEL32.NewProc("GetSystemWow64DirectoryA")
	procGetTickCount                       = modKERNEL32.NewProc("GetTickCount")
	procGetTickCount64                     = modKERNEL32.NewProc("GetTickCount64")
	procGetVersion                         = modKERNEL32.NewProc("GetVersion")
	procGetVersionEx                       = modKERNEL32.NewProc("GetVersionExW")
	procGetVersionExA                      = modKERNEL32.NewProc("GetVersionExA")
	procGetWindowsDirectory                = modKERNEL32.NewProc("GetWindowsDirectoryW")
	procGetWindowsDirectoryA               = modKERNEL32.NewProc("GetWindowsDirectoryA")
	procGlobalMemoryStatus                 = modKERNEL32.NewProc("GlobalMemoryStatus")
	procGlobalMemoryStatusEx               = modKERNEL32.NewProc("GlobalMemoryStatusEx")
	procIsUserCetAvailableInEnvironment    = modKERNEL32.NewProc("IsUserCetAvailableInEnvironment")
	procIsWow64GuestMachineSupported       = modKERNEL32.NewProc("IsWow64GuestMachineSupported")
	procSetComputerName                    = modKERNEL32.NewProc("SetComputerNameW")
	procSetComputerNameA                   = modKERNEL32.NewProc("SetComputerNameA")
	procSetComputerNameEx                  = modKERNEL32.NewProc("SetComputerNameExW")
	procSetComputerNameEx2W                = modKERNEL32.NewProc("SetComputerNameEx2W")
	procSetComputerNameExA                 = modKERNEL32.NewProc("SetComputerNameExA")
	procSetLocalTime                       = modKERNEL32.NewProc("SetLocalTime")
	procSetSystemTime                      = modKERNEL32.NewProc("SetSystemTime")
	procSetSystemTimeAdjustment            = modKERNEL32.NewProc("SetSystemTimeAdjustment")
	procVerSetConditionMask                = modKERNEL32.NewProc("VerSetConditionMask")
	procVerifyVersionInfo                  = modKERNEL32.NewProc("VerifyVersionInfoW")
	procVerifyVersionInfoA                 = modKERNEL32.NewProc("VerifyVersionInfoA")
	procRtlConvertDeviceFamilyInfoToString = modntdll.NewProc("RtlConvertDeviceFamilyInfoToString")
	procRtlGetDeviceFamilyInfoEnum         = modntdll.NewProc("RtlGetDeviceFamilyInfoEnum")
	procRtlGetProductInfo                  = modntdll.NewProc("RtlGetProductInfo")
	procRtlOsDeploymentState               = modntdll.NewProc("RtlOsDeploymentState")
	procRtlSwitchedVVI                     = modntdll.NewProc("RtlSwitchedVVI")
	procRtlGetSystemGlobalData             = modntdllk.NewProc("RtlGetSystemGlobalData")
)

// Procs exposes this package's lazily resolved exports for availability
// probing: Procs.<Function>.Find() reports nil, or the *win32.ProcError a
// call to <Function> would panic with on this system (an export missing from
// this Windows build, or a DLL that is not installed).
var Procs = struct {
	DnsHostnameToComputerNameExW       *win32.Proc
	EnumSystemFirmwareTables           *win32.Proc
	GetComputerNameEx                  *win32.Proc
	GetComputerNameExA                 *win32.Proc
	GetDeveloperDriveEnablementState   *win32.Proc
	GetFirmwareType                    *win32.Proc
	GetIntegratedDisplaySize           *win32.Proc
	GetLocalTime                       *win32.Proc
	GetLogicalProcessorInformation     *win32.Proc
	GetLogicalProcessorInformationEx   *win32.Proc
	GetNativeSystemInfo                *win32.Proc
	GetOsManufacturingMode             *win32.Proc
	GetOsSafeBootMode                  *win32.Proc
	GetPhysicallyInstalledSystemMemory *win32.Proc
	GetProcessorSystemCycleTime        *win32.Proc
	GetProductInfo                     *win32.Proc
	GetRuntimeAttestationReport        *win32.Proc
	GetSystemCpuSetInformation         *win32.Proc
	GetSystemDEPPolicy                 *win32.Proc
	GetSystemDirectory                 *win32.Proc
	GetSystemDirectoryA                *win32.Proc
	GetSystemFirmwareTable             *win32.Proc
	GetSystemInfo                      *win32.Proc
	GetSystemLeapSecondInformation     *win32.Proc
	GetSystemTime                      *win32.Proc
	GetSystemTimeAdjustment            *win32.Proc
	GetSystemTimeAdjustmentPrecise     *win32.Proc
	GetSystemTimeAsFileTime            *win32.Proc
	GetSystemTimePreciseAsFileTime     *win32.Proc
	GetSystemWindowsDirectory          *win32.Proc
	GetSystemWindowsDirectoryA         *win32.Proc
	GetSystemWow64Directory            *win32.Proc
	GetSystemWow64Directory2           *win32.Proc
	GetSystemWow64Directory2A          *win32.Proc
	GetSystemWow64DirectoryA           *win32.Proc
	GetTickCount                       *win32.Proc
	GetTickCount64                     *win32.Proc
	GetVersion                         *win32.Proc
	GetVersionEx                       *win32.Proc
	GetVersionExA                      *win32.Proc
	GetWindowsDirectory                *win32.Proc
	GetWindowsDirectoryA               *win32.Proc
	GlobalMemoryStatus                 *win32.Proc
	GlobalMemoryStatusEx               *win32.Proc
	IsUserCetAvailableInEnvironment    *win32.Proc
	IsWow64GuestMachineSupported       *win32.Proc
	RtlConvertDeviceFamilyInfoToString *win32.Proc
	RtlGetDeviceFamilyInfoEnum         *win32.Proc
	RtlGetProductInfo                  *win32.Proc
	RtlGetSystemGlobalData             *win32.Proc
	RtlOsDeploymentState               *win32.Proc
	RtlSwitchedVVI                     *win32.Proc
	SetComputerName                    *win32.Proc
	SetComputerNameA                   *win32.Proc
	SetComputerNameEx                  *win32.Proc
	SetComputerNameEx2W                *win32.Proc
	SetComputerNameExA                 *win32.Proc
	SetLocalTime                       *win32.Proc
	SetSystemTime                      *win32.Proc
	SetSystemTimeAdjustment            *win32.Proc
	SetSystemTimeAdjustmentPrecise     *win32.Proc
	VerSetConditionMask                *win32.Proc
	VerifyVersionInfo                  *win32.Proc
	VerifyVersionInfoA                 *win32.Proc
}{
	DnsHostnameToComputerNameExW:       procDnsHostnameToComputerNameExW,
	EnumSystemFirmwareTables:           procEnumSystemFirmwareTables,
	GetComputerNameEx:                  procGetComputerNameEx,
	GetComputerNameExA:                 procGetComputerNameExA,
	GetDeveloperDriveEnablementState:   procGetDeveloperDriveEnablementState,
	GetFirmwareType:                    procGetFirmwareType,
	GetIntegratedDisplaySize:           procGetIntegratedDisplaySize,
	GetLocalTime:                       procGetLocalTime,
	GetLogicalProcessorInformation:     procGetLogicalProcessorInformation,
	GetLogicalProcessorInformationEx:   procGetLogicalProcessorInformationEx,
	GetNativeSystemInfo:                procGetNativeSystemInfo,
	GetOsManufacturingMode:             procGetOsManufacturingMode,
	GetOsSafeBootMode:                  procGetOsSafeBootMode,
	GetPhysicallyInstalledSystemMemory: procGetPhysicallyInstalledSystemMemory,
	GetProcessorSystemCycleTime:        procGetProcessorSystemCycleTime,
	GetProductInfo:                     procGetProductInfo,
	GetRuntimeAttestationReport:        procGetRuntimeAttestationReport,
	GetSystemCpuSetInformation:         procGetSystemCpuSetInformation,
	GetSystemDEPPolicy:                 procGetSystemDEPPolicy,
	GetSystemDirectory:                 procGetSystemDirectory,
	GetSystemDirectoryA:                procGetSystemDirectoryA,
	GetSystemFirmwareTable:             procGetSystemFirmwareTable,
	GetSystemInfo:                      procGetSystemInfo,
	GetSystemLeapSecondInformation:     procGetSystemLeapSecondInformation,
	GetSystemTime:                      procGetSystemTime,
	GetSystemTimeAdjustment:            procGetSystemTimeAdjustment,
	GetSystemTimeAdjustmentPrecise:     procGetSystemTimeAdjustmentPrecise,
	GetSystemTimeAsFileTime:            procGetSystemTimeAsFileTime,
	GetSystemTimePreciseAsFileTime:     procGetSystemTimePreciseAsFileTime,
	GetSystemWindowsDirectory:          procGetSystemWindowsDirectory,
	GetSystemWindowsDirectoryA:         procGetSystemWindowsDirectoryA,
	GetSystemWow64Directory:            procGetSystemWow64Directory,
	GetSystemWow64Directory2:           procGetSystemWow64Directory2,
	GetSystemWow64Directory2A:          procGetSystemWow64Directory2A,
	GetSystemWow64DirectoryA:           procGetSystemWow64DirectoryA,
	GetTickCount:                       procGetTickCount,
	GetTickCount64:                     procGetTickCount64,
	GetVersion:                         procGetVersion,
	GetVersionEx:                       procGetVersionEx,
	GetVersionExA:                      procGetVersionExA,
	GetWindowsDirectory:                procGetWindowsDirectory,
	GetWindowsDirectoryA:               procGetWindowsDirectoryA,
	GlobalMemoryStatus:                 procGlobalMemoryStatus,
	GlobalMemoryStatusEx:               procGlobalMemoryStatusEx,
	IsUserCetAvailableInEnvironment:    procIsUserCetAvailableInEnvironment,
	IsWow64GuestMachineSupported:       procIsWow64GuestMachineSupported,
	RtlConvertDeviceFamilyInfoToString: procRtlConvertDeviceFamilyInfoToString,
	RtlGetDeviceFamilyInfoEnum:         procRtlGetDeviceFamilyInfoEnum,
	RtlGetProductInfo:                  procRtlGetProductInfo,
	RtlGetSystemGlobalData:             procRtlGetSystemGlobalData,
	RtlOsDeploymentState:               procRtlOsDeploymentState,
	RtlSwitchedVVI:                     procRtlSwitchedVVI,
	SetComputerName:                    procSetComputerName,
	SetComputerNameA:                   procSetComputerNameA,
	SetComputerNameEx:                  procSetComputerNameEx,
	SetComputerNameEx2W:                procSetComputerNameEx2W,
	SetComputerNameExA:                 procSetComputerNameExA,
	SetLocalTime:                       procSetLocalTime,
	SetSystemTime:                      procSetSystemTime,
	SetSystemTimeAdjustment:            procSetSystemTimeAdjustment,
	SetSystemTimeAdjustmentPrecise:     procSetSystemTimeAdjustmentPrecise,
	VerSetConditionMask:                procVerSetConditionMask,
	VerifyVersionInfo:                  procVerifyVersionInfo,
	VerifyVersionInfoA:                 procVerifyVersionInfoA,
}

// DnsHostnameToComputerNameExW calls KERNEL32!DnsHostnameToComputerNameExW.
func DnsHostnameToComputerNameExW(Hostname string, ComputerName foundation.PWSTR, nSize *uint32) bool {
	_Hostname := win32.UTF16Ptr(Hostname)
	r1, _, _ := syscall.SyscallN(procDnsHostnameToComputerNameExW.Addr(), uintptr(unsafe.Pointer(_Hostname)), uintptr(unsafe.Pointer(ComputerName)), uintptr(unsafe.Pointer(nSize)))
	return r1 != 0
}

// EnumSystemFirmwareTables calls KERNEL32!EnumSystemFirmwareTables.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-enumsystemfirmwaretables
// Minimum OS: windows6.0.6000.
func EnumSystemFirmwareTables(FirmwareTableProviderSignature FIRMWARE_TABLE_PROVIDER, pFirmwareTableEnumBuffer []byte) (uint32, error) {
	var _pFirmwareTableEnumBuffer *byte
	if len(pFirmwareTableEnumBuffer) > 0 {
		_pFirmwareTableEnumBuffer = &pFirmwareTableEnumBuffer[0]
	}
	r1, _, e1 := syscall.SyscallN(procEnumSystemFirmwareTables.Addr(), uintptr(FirmwareTableProviderSignature), uintptr(unsafe.Pointer(_pFirmwareTableEnumBuffer)), uintptr(len(pFirmwareTableEnumBuffer)))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetComputerNameEx calls KERNEL32!GetComputerNameExW.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getcomputernameexw
// Minimum OS: windows5.0.
func GetComputerNameEx(NameType COMPUTER_NAME_FORMAT, lpBuffer foundation.PWSTR, nSize *uint32) error {
	r1, _, e1 := syscall.SyscallN(procGetComputerNameEx.Addr(), uintptr(NameType), uintptr(unsafe.Pointer(lpBuffer)), uintptr(unsafe.Pointer(nSize)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetComputerNameExA calls KERNEL32!GetComputerNameExA.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getcomputernameexa
// Minimum OS: windows5.0.
func GetComputerNameExA(NameType COMPUTER_NAME_FORMAT, lpBuffer foundation.PSTR, nSize *uint32) error {
	r1, _, e1 := syscall.SyscallN(procGetComputerNameExA.Addr(), uintptr(NameType), uintptr(unsafe.Pointer(lpBuffer)), uintptr(unsafe.Pointer(nSize)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetDeveloperDriveEnablementState calls api-ms-win-core-sysinfo-l1-2-6!GetDeveloperDriveEnablementState.
func GetDeveloperDriveEnablementState() DEVELOPER_DRIVE_ENABLEMENT_STATE {
	r1, _, _ := syscall.SyscallN(procGetDeveloperDriveEnablementState.Addr())
	return DEVELOPER_DRIVE_ENABLEMENT_STATE(r1)
}

// GetFirmwareType calls KERNEL32!GetFirmwareType.
// https://learn.microsoft.com/windows/win32/api/winbase/nf-winbase-getfirmwaretype
// Minimum OS: windows8.0.
func GetFirmwareType(FirmwareType *FIRMWARE_TYPE) error {
	r1, _, e1 := syscall.SyscallN(procGetFirmwareType.Addr(), uintptr(unsafe.Pointer(FirmwareType)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetIntegratedDisplaySize calls api-ms-win-core-sysinfo-l1-2-3!GetIntegratedDisplaySize.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getintegrateddisplaysize
// Minimum OS: windows10.0.10240.
func GetIntegratedDisplaySize(sizeInInches *float64) error {
	r1, _, _ := syscall.SyscallN(procGetIntegratedDisplaySize.Addr(), uintptr(unsafe.Pointer(sizeInInches)))
	return win32.ErrIfFailed(int32(r1))
}

// GetLocalTime calls KERNEL32!GetLocalTime.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getlocaltime
// Minimum OS: windows5.0.
func GetLocalTime(lpSystemTime *foundation.SYSTEMTIME) {
	syscall.SyscallN(procGetLocalTime.Addr(), uintptr(unsafe.Pointer(lpSystemTime)))
}

// GetLogicalProcessorInformation calls KERNEL32!GetLogicalProcessorInformation.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getlogicalprocessorinformation
// Minimum OS: windows6.0.6000.
func GetLogicalProcessorInformation(Buffer *SYSTEM_LOGICAL_PROCESSOR_INFORMATION, ReturnedLength *uint32) error {
	r1, _, e1 := syscall.SyscallN(procGetLogicalProcessorInformation.Addr(), uintptr(unsafe.Pointer(Buffer)), uintptr(unsafe.Pointer(ReturnedLength)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetLogicalProcessorInformationEx calls KERNEL32!GetLogicalProcessorInformationEx.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getlogicalprocessorinformationex
// Minimum OS: windows6.1.
func GetLogicalProcessorInformationEx(RelationshipType LOGICAL_PROCESSOR_RELATIONSHIP, Buffer *SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX, ReturnedLength *uint32) error {
	r1, _, e1 := syscall.SyscallN(procGetLogicalProcessorInformationEx.Addr(), uintptr(RelationshipType), uintptr(unsafe.Pointer(Buffer)), uintptr(unsafe.Pointer(ReturnedLength)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetNativeSystemInfo calls KERNEL32!GetNativeSystemInfo.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getnativesysteminfo
// Minimum OS: windows5.1.2600.
func GetNativeSystemInfo(lpSystemInfo *SYSTEM_INFO) {
	syscall.SyscallN(procGetNativeSystemInfo.Addr(), uintptr(unsafe.Pointer(lpSystemInfo)))
}

// GetOsManufacturingMode calls api-ms-win-core-sysinfo-l1-2-3!GetOsManufacturingMode.
func GetOsManufacturingMode(pbEnabled *foundation.BOOL) bool {
	r1, _, _ := syscall.SyscallN(procGetOsManufacturingMode.Addr(), uintptr(unsafe.Pointer(pbEnabled)))
	return r1 != 0
}

// GetOsSafeBootMode calls api-ms-win-core-sysinfo-l1-2-0!GetOsSafeBootMode.
func GetOsSafeBootMode(Flags *uint32) bool {
	r1, _, _ := syscall.SyscallN(procGetOsSafeBootMode.Addr(), uintptr(unsafe.Pointer(Flags)))
	return r1 != 0
}

// GetPhysicallyInstalledSystemMemory calls KERNEL32!GetPhysicallyInstalledSystemMemory.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getphysicallyinstalledsystemmemory
// Minimum OS: windows6.0.6000.
func GetPhysicallyInstalledSystemMemory(TotalMemoryInKilobytes *uint64) error {
	r1, _, e1 := syscall.SyscallN(procGetPhysicallyInstalledSystemMemory.Addr(), uintptr(unsafe.Pointer(TotalMemoryInKilobytes)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetProcessorSystemCycleTime calls KERNEL32!GetProcessorSystemCycleTime.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getprocessorsystemcycletime
// Minimum OS: windows6.1.
func GetProcessorSystemCycleTime(Group uint16, Buffer *SYSTEM_PROCESSOR_CYCLE_TIME_INFORMATION, ReturnedLength *uint32) error {
	r1, _, e1 := syscall.SyscallN(procGetProcessorSystemCycleTime.Addr(), uintptr(Group), uintptr(unsafe.Pointer(Buffer)), uintptr(unsafe.Pointer(ReturnedLength)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetProductInfo calls KERNEL32!GetProductInfo.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getproductinfo
// Minimum OS: windows6.0.6000.
func GetProductInfo(dwOSMajorVersion uint32, dwOSMinorVersion uint32, dwSpMajorVersion uint32, dwSpMinorVersion uint32, pdwReturnedProductType *OS_PRODUCT_TYPE) bool {
	r1, _, _ := syscall.SyscallN(procGetProductInfo.Addr(), uintptr(dwOSMajorVersion), uintptr(dwOSMinorVersion), uintptr(dwSpMajorVersion), uintptr(dwSpMinorVersion), uintptr(unsafe.Pointer(pdwReturnedProductType)))
	return r1 != 0
}

// GetRuntimeAttestationReport calls KERNEL32!GetRuntimeAttestationReport.
func GetRuntimeAttestationReport(Nonce *byte, PackageVersion uint16, ReportTypesBitmap uint64, ReportBuffer unsafe.Pointer, ReportBufferSize *uint32) bool {
	r1, _, _ := syscall.SyscallN(procGetRuntimeAttestationReport.Addr(), uintptr(unsafe.Pointer(Nonce)), uintptr(PackageVersion), uintptr(ReportTypesBitmap), uintptr(unsafe.Pointer(ReportBuffer)), uintptr(unsafe.Pointer(ReportBufferSize)))
	return r1 != 0
}

// GetSystemCpuSetInformation calls KERNEL32!GetSystemCpuSetInformation.
// https://learn.microsoft.com/windows/win32/ProcThread/getsystemcpusetinformation
func GetSystemCpuSetInformation(Information *SYSTEM_CPU_SET_INFORMATION, BufferLength uint32, ReturnedLength *uint32, Process foundation.HANDLE) bool {
	r1, _, _ := syscall.SyscallN(procGetSystemCpuSetInformation.Addr(), uintptr(unsafe.Pointer(Information)), uintptr(BufferLength), uintptr(unsafe.Pointer(ReturnedLength)), uintptr(Process), 0)
	return r1 != 0
}

// GetSystemDEPPolicy calls KERNEL32!GetSystemDEPPolicy.
// https://learn.microsoft.com/windows/win32/api/winbase/nf-winbase-getsystemdeppolicy
// Minimum OS: windows6.0.6000.
func GetSystemDEPPolicy() DEP_SYSTEM_POLICY_TYPE {
	r1, _, _ := syscall.SyscallN(procGetSystemDEPPolicy.Addr())
	return DEP_SYSTEM_POLICY_TYPE(r1)
}

// GetSystemDirectory calls KERNEL32!GetSystemDirectoryW.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemdirectoryw
// Minimum OS: windows5.0.
func GetSystemDirectory(lpBuffer foundation.PWSTR, uSize uint32) (uint32, error) {
	r1, _, e1 := syscall.SyscallN(procGetSystemDirectory.Addr(), uintptr(unsafe.Pointer(lpBuffer)), uintptr(uSize))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetSystemDirectoryA calls KERNEL32!GetSystemDirectoryA.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemdirectorya
// Minimum OS: windows5.0.
func GetSystemDirectoryA(lpBuffer foundation.PSTR, uSize uint32) (uint32, error) {
	r1, _, e1 := syscall.SyscallN(procGetSystemDirectoryA.Addr(), uintptr(unsafe.Pointer(lpBuffer)), uintptr(uSize))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetSystemFirmwareTable calls KERNEL32!GetSystemFirmwareTable.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemfirmwaretable
// Minimum OS: windows6.0.6000.
func GetSystemFirmwareTable(FirmwareTableProviderSignature FIRMWARE_TABLE_PROVIDER, FirmwareTableID uint32, pFirmwareTableBuffer []byte) (uint32, error) {
	var _pFirmwareTableBuffer *byte
	if len(pFirmwareTableBuffer) > 0 {
		_pFirmwareTableBuffer = &pFirmwareTableBuffer[0]
	}
	r1, _, e1 := syscall.SyscallN(procGetSystemFirmwareTable.Addr(), uintptr(FirmwareTableProviderSignature), uintptr(FirmwareTableID), uintptr(unsafe.Pointer(_pFirmwareTableBuffer)), uintptr(len(pFirmwareTableBuffer)))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetSystemInfo calls KERNEL32!GetSystemInfo.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsysteminfo
// Minimum OS: windows5.0.
func GetSystemInfo(lpSystemInfo *SYSTEM_INFO) {
	syscall.SyscallN(procGetSystemInfo.Addr(), uintptr(unsafe.Pointer(lpSystemInfo)))
}

// GetSystemLeapSecondInformation calls KERNEL32!GetSystemLeapSecondInformation.
func GetSystemLeapSecondInformation(Enabled *foundation.BOOL, Flags *uint32) bool {
	r1, _, _ := syscall.SyscallN(procGetSystemLeapSecondInformation.Addr(), uintptr(unsafe.Pointer(Enabled)), uintptr(unsafe.Pointer(Flags)))
	return r1 != 0
}

// GetSystemTime calls KERNEL32!GetSystemTime.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemtime
// Minimum OS: windows5.0.
func GetSystemTime(lpSystemTime *foundation.SYSTEMTIME) {
	syscall.SyscallN(procGetSystemTime.Addr(), uintptr(unsafe.Pointer(lpSystemTime)))
}

// GetSystemTimeAdjustment calls KERNEL32!GetSystemTimeAdjustment.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemtimeadjustment
// Minimum OS: windows5.0.
func GetSystemTimeAdjustment(lpTimeAdjustment *uint32, lpTimeIncrement *uint32, lpTimeAdjustmentDisabled *foundation.BOOL) error {
	r1, _, e1 := syscall.SyscallN(procGetSystemTimeAdjustment.Addr(), uintptr(unsafe.Pointer(lpTimeAdjustment)), uintptr(unsafe.Pointer(lpTimeIncrement)), uintptr(unsafe.Pointer(lpTimeAdjustmentDisabled)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetSystemTimeAdjustmentPrecise calls api-ms-win-core-sysinfo-l1-2-4!GetSystemTimeAdjustmentPrecise.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemtimeadjustmentprecise
// Minimum OS: windows10.0.10240.
func GetSystemTimeAdjustmentPrecise(lpTimeAdjustment *uint64, lpTimeIncrement *uint64, lpTimeAdjustmentDisabled *foundation.BOOL) error {
	r1, _, e1 := syscall.SyscallN(procGetSystemTimeAdjustmentPrecise.Addr(), uintptr(unsafe.Pointer(lpTimeAdjustment)), uintptr(unsafe.Pointer(lpTimeIncrement)), uintptr(unsafe.Pointer(lpTimeAdjustmentDisabled)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetSystemTimeAsFileTime calls KERNEL32!GetSystemTimeAsFileTime.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemtimeasfiletime
// Minimum OS: windows5.0.
func GetSystemTimeAsFileTime(lpSystemTimeAsFileTime *foundation.FILETIME) {
	syscall.SyscallN(procGetSystemTimeAsFileTime.Addr(), uintptr(unsafe.Pointer(lpSystemTimeAsFileTime)))
}

// GetSystemTimePreciseAsFileTime calls KERNEL32!GetSystemTimePreciseAsFileTime.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemtimepreciseasfiletime
// Minimum OS: windows8.0.
func GetSystemTimePreciseAsFileTime(lpSystemTimeAsFileTime *foundation.FILETIME) {
	syscall.SyscallN(procGetSystemTimePreciseAsFileTime.Addr(), uintptr(unsafe.Pointer(lpSystemTimeAsFileTime)))
}

// GetSystemWindowsDirectory calls KERNEL32!GetSystemWindowsDirectoryW.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemwindowsdirectoryw
// Minimum OS: windows5.0.
func GetSystemWindowsDirectory(lpBuffer foundation.PWSTR, uSize uint32) (uint32, error) {
	r1, _, e1 := syscall.SyscallN(procGetSystemWindowsDirectory.Addr(), uintptr(unsafe.Pointer(lpBuffer)), uintptr(uSize))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetSystemWindowsDirectoryA calls KERNEL32!GetSystemWindowsDirectoryA.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getsystemwindowsdirectorya
// Minimum OS: windows5.0.
func GetSystemWindowsDirectoryA(lpBuffer foundation.PSTR, uSize uint32) (uint32, error) {
	r1, _, e1 := syscall.SyscallN(procGetSystemWindowsDirectoryA.Addr(), uintptr(unsafe.Pointer(lpBuffer)), uintptr(uSize))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetSystemWow64Directory calls KERNEL32!GetSystemWow64DirectoryW.
// https://learn.microsoft.com/windows/win32/api/wow64apiset/nf-wow64apiset-getsystemwow64directoryw
// Minimum OS: windows5.1.2600.
func GetSystemWow64Directory(lpBuffer foundation.PWSTR, uSize uint32) (uint32, error) {
	r1, _, e1 := syscall.SyscallN(procGetSystemWow64Directory.Addr(), uintptr(unsafe.Pointer(lpBuffer)), uintptr(uSize))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetSystemWow64Directory2 calls api-ms-win-core-wow64-l1-1-1!GetSystemWow64Directory2W.
// https://learn.microsoft.com/windows/win32/api/wow64apiset/nf-wow64apiset-getsystemwow64directory2w
// Minimum OS: windows10.0.10586.
func GetSystemWow64Directory2(lpBuffer foundation.PWSTR, uSize uint32, ImageFileMachineType IMAGE_FILE_MACHINE) (uint32, error) {
	r1, _, e1 := syscall.SyscallN(procGetSystemWow64Directory2.Addr(), uintptr(unsafe.Pointer(lpBuffer)), uintptr(uSize), uintptr(ImageFileMachineType))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetSystemWow64Directory2A calls api-ms-win-core-wow64-l1-1-1!GetSystemWow64Directory2A.
// https://learn.microsoft.com/windows/win32/api/wow64apiset/nf-wow64apiset-getsystemwow64directory2a
// Minimum OS: windows10.0.10586.
func GetSystemWow64Directory2A(lpBuffer foundation.PSTR, uSize uint32, ImageFileMachineType IMAGE_FILE_MACHINE) (uint32, error) {
	r1, _, e1 := syscall.SyscallN(procGetSystemWow64Directory2A.Addr(), uintptr(unsafe.Pointer(lpBuffer)), uintptr(uSize), uintptr(ImageFileMachineType))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetSystemWow64DirectoryA calls KERNEL32!GetSystemWow64DirectoryA.
// https://learn.microsoft.com/windows/win32/api/wow64apiset/nf-wow64apiset-getsystemwow64directorya
// Minimum OS: windows5.1.2600.
func GetSystemWow64DirectoryA(lpBuffer foundation.PSTR, uSize uint32) (uint32, error) {
	r1, _, e1 := syscall.SyscallN(procGetSystemWow64DirectoryA.Addr(), uintptr(unsafe.Pointer(lpBuffer)), uintptr(uSize))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetTickCount calls KERNEL32!GetTickCount.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-gettickcount
// Minimum OS: windows5.0.
func GetTickCount() uint32 {
	r1, _, _ := syscall.SyscallN(procGetTickCount.Addr())
	return uint32(r1)
}

// GetTickCount64 calls KERNEL32!GetTickCount64.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-gettickcount64
// Minimum OS: windows6.0.6000.
func GetTickCount64() uint64 {
	r1, _, _ := syscall.SyscallN(procGetTickCount64.Addr())
	return uint64(r1)
}

// GetVersion calls KERNEL32!GetVersion.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getversion
// Minimum OS: windows5.0.
func GetVersion() uint32 {
	r1, _, _ := syscall.SyscallN(procGetVersion.Addr())
	return uint32(r1)
}

// GetVersionEx calls KERNEL32!GetVersionExW.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getversionexw
// Minimum OS: windows5.0.
func GetVersionEx(lpVersionInformation *OSVERSIONINFOW) error {
	r1, _, e1 := syscall.SyscallN(procGetVersionEx.Addr(), uintptr(unsafe.Pointer(lpVersionInformation)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetVersionExA calls KERNEL32!GetVersionExA.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getversionexa
// Minimum OS: windows5.0.
func GetVersionExA(lpVersionInformation *OSVERSIONINFOA) error {
	r1, _, e1 := syscall.SyscallN(procGetVersionExA.Addr(), uintptr(unsafe.Pointer(lpVersionInformation)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// GetWindowsDirectory calls KERNEL32!GetWindowsDirectoryW.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getwindowsdirectoryw
// Minimum OS: windows5.0.
func GetWindowsDirectory(lpBuffer foundation.PWSTR, uSize uint32) (uint32, error) {
	r1, _, e1 := syscall.SyscallN(procGetWindowsDirectory.Addr(), uintptr(unsafe.Pointer(lpBuffer)), uintptr(uSize))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GetWindowsDirectoryA calls KERNEL32!GetWindowsDirectoryA.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-getwindowsdirectorya
// Minimum OS: windows5.0.
func GetWindowsDirectoryA(lpBuffer foundation.PSTR, uSize uint32) (uint32, error) {
	r1, _, e1 := syscall.SyscallN(procGetWindowsDirectoryA.Addr(), uintptr(unsafe.Pointer(lpBuffer)), uintptr(uSize))
	if e1 != 0 {
		return uint32(r1), e1
	}
	return uint32(r1), nil
}

// GlobalMemoryStatus calls KERNEL32!GlobalMemoryStatus.
// https://learn.microsoft.com/windows/win32/api/winbase/nf-winbase-globalmemorystatus
// Minimum OS: windows5.1.2600.
func GlobalMemoryStatus(lpBuffer *MEMORYSTATUS) {
	syscall.SyscallN(procGlobalMemoryStatus.Addr(), uintptr(unsafe.Pointer(lpBuffer)))
}

// GlobalMemoryStatusEx calls KERNEL32!GlobalMemoryStatusEx.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-globalmemorystatusex
// Minimum OS: windows5.1.2600.
func GlobalMemoryStatusEx(lpBuffer *MEMORYSTATUSEX) error {
	r1, _, e1 := syscall.SyscallN(procGlobalMemoryStatusEx.Addr(), uintptr(unsafe.Pointer(lpBuffer)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// IsUserCetAvailableInEnvironment calls KERNEL32!IsUserCetAvailableInEnvironment.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-isusercetavailableinenvironment
func IsUserCetAvailableInEnvironment(UserCetEnvironment USER_CET_ENVIRONMENT) bool {
	r1, _, _ := syscall.SyscallN(procIsUserCetAvailableInEnvironment.Addr(), uintptr(UserCetEnvironment))
	return r1 != 0
}

// IsWow64GuestMachineSupported calls KERNEL32!IsWow64GuestMachineSupported.
// https://learn.microsoft.com/windows/win32/api/wow64apiset/nf-wow64apiset-iswow64guestmachinesupported
// Minimum OS: windows10.0.16299.
func IsWow64GuestMachineSupported(WowGuestMachine IMAGE_FILE_MACHINE, MachineIsSupported *foundation.BOOL) error {
	r1, _, _ := syscall.SyscallN(procIsWow64GuestMachineSupported.Addr(), uintptr(WowGuestMachine), uintptr(unsafe.Pointer(MachineIsSupported)))
	return win32.ErrIfFailed(int32(r1))
}

// RtlConvertDeviceFamilyInfoToString calls ntdll!RtlConvertDeviceFamilyInfoToString.
// https://learn.microsoft.com/windows/win32/api/winnt/nf-winnt-rtlconvertdevicefamilyinfotostring
func RtlConvertDeviceFamilyInfoToString(pulDeviceFamilyBufferSize *uint32, pulDeviceFormBufferSize *uint32, DeviceFamily foundation.PWSTR, DeviceForm foundation.PWSTR) uint32 {
	r1, _, _ := syscall.SyscallN(procRtlConvertDeviceFamilyInfoToString.Addr(), uintptr(unsafe.Pointer(pulDeviceFamilyBufferSize)), uintptr(unsafe.Pointer(pulDeviceFormBufferSize)), uintptr(unsafe.Pointer(DeviceFamily)), uintptr(unsafe.Pointer(DeviceForm)))
	return uint32(r1)
}

// RtlGetDeviceFamilyInfoEnum calls ntdll!RtlGetDeviceFamilyInfoEnum.
// https://learn.microsoft.com/windows/win32/DevNotes/rtlgetdevicefamilyinfoenum
func RtlGetDeviceFamilyInfoEnum(pullUAPInfo *uint64, pulDeviceFamily *DEVICEFAMILYINFOENUM, pulDeviceForm *DEVICEFAMILYDEVICEFORM) {
	syscall.SyscallN(procRtlGetDeviceFamilyInfoEnum.Addr(), uintptr(unsafe.Pointer(pullUAPInfo)), uintptr(unsafe.Pointer(pulDeviceFamily)), uintptr(unsafe.Pointer(pulDeviceForm)))
}

// RtlGetProductInfo calls ntdll!RtlGetProductInfo.
func RtlGetProductInfo(OSMajorVersion uint32, OSMinorVersion uint32, SpMajorVersion uint32, SpMinorVersion uint32, ReturnedProductType *uint32) foundation.BOOLEAN {
	r1, _, _ := syscall.SyscallN(procRtlGetProductInfo.Addr(), uintptr(OSMajorVersion), uintptr(OSMinorVersion), uintptr(SpMajorVersion), uintptr(SpMinorVersion), uintptr(unsafe.Pointer(ReturnedProductType)))
	return foundation.BOOLEAN(r1)
}

// RtlGetSystemGlobalData calls ntdllk!RtlGetSystemGlobalData.
func RtlGetSystemGlobalData(DataId RTL_SYSTEM_GLOBAL_DATA_ID, Buffer unsafe.Pointer, Size uint32) uint32 {
	r1, _, _ := syscall.SyscallN(procRtlGetSystemGlobalData.Addr(), uintptr(DataId), uintptr(unsafe.Pointer(Buffer)), uintptr(Size))
	return uint32(r1)
}

// RtlOsDeploymentState calls ntdll!RtlOsDeploymentState.
func RtlOsDeploymentState(Flags uint32) OS_DEPLOYEMENT_STATE_VALUES {
	r1, _, _ := syscall.SyscallN(procRtlOsDeploymentState.Addr(), uintptr(Flags))
	return OS_DEPLOYEMENT_STATE_VALUES(r1)
}

// RtlSwitchedVVI calls ntdll!RtlSwitchedVVI.
func RtlSwitchedVVI(VersionInfo *OSVERSIONINFOEXW, TypeMask uint32, ConditionMask uint64) uint32 {
	r1, _, _ := syscall.SyscallN(procRtlSwitchedVVI.Addr(), uintptr(unsafe.Pointer(VersionInfo)), uintptr(TypeMask), uintptr(ConditionMask))
	return uint32(r1)
}

// SetComputerName calls KERNEL32!SetComputerNameW.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-setcomputernamew
// Minimum OS: windows5.0.
func SetComputerName(lpComputerName string) error {
	_lpComputerName := win32.UTF16Ptr(lpComputerName)
	r1, _, e1 := syscall.SyscallN(procSetComputerName.Addr(), uintptr(unsafe.Pointer(_lpComputerName)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// SetComputerNameA calls KERNEL32!SetComputerNameA.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-setcomputernamea
// Minimum OS: windows5.0.
func SetComputerNameA(lpComputerName foundation.PSTR) error {
	r1, _, e1 := syscall.SyscallN(procSetComputerNameA.Addr(), uintptr(unsafe.Pointer(lpComputerName)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// SetComputerNameEx calls KERNEL32!SetComputerNameExW.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-setcomputernameexw
// Minimum OS: windows5.0.
func SetComputerNameEx(NameType COMPUTER_NAME_FORMAT, lpBuffer string) error {
	_lpBuffer := win32.UTF16Ptr(lpBuffer)
	r1, _, e1 := syscall.SyscallN(procSetComputerNameEx.Addr(), uintptr(NameType), uintptr(unsafe.Pointer(_lpBuffer)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// SetComputerNameEx2W calls KERNEL32!SetComputerNameEx2W.
func SetComputerNameEx2W(NameType COMPUTER_NAME_FORMAT, Flags uint32, lpBuffer string) bool {
	_lpBuffer := win32.UTF16Ptr(lpBuffer)
	r1, _, _ := syscall.SyscallN(procSetComputerNameEx2W.Addr(), uintptr(NameType), uintptr(Flags), uintptr(unsafe.Pointer(_lpBuffer)))
	return r1 != 0
}

// SetComputerNameExA calls KERNEL32!SetComputerNameExA.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-setcomputernameexa
// Minimum OS: windows5.0.
func SetComputerNameExA(NameType COMPUTER_NAME_FORMAT, lpBuffer foundation.PSTR) error {
	r1, _, e1 := syscall.SyscallN(procSetComputerNameExA.Addr(), uintptr(NameType), uintptr(unsafe.Pointer(lpBuffer)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// SetLocalTime calls KERNEL32!SetLocalTime.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-setlocaltime
// Minimum OS: windows5.0.
func SetLocalTime(lpSystemTime *foundation.SYSTEMTIME) error {
	r1, _, e1 := syscall.SyscallN(procSetLocalTime.Addr(), uintptr(unsafe.Pointer(lpSystemTime)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// SetSystemTime calls KERNEL32!SetSystemTime.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-setsystemtime
// Minimum OS: windows5.0.
func SetSystemTime(lpSystemTime *foundation.SYSTEMTIME) error {
	r1, _, e1 := syscall.SyscallN(procSetSystemTime.Addr(), uintptr(unsafe.Pointer(lpSystemTime)))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// SetSystemTimeAdjustment calls KERNEL32!SetSystemTimeAdjustment.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-setsystemtimeadjustment
// Minimum OS: windows5.0.
func SetSystemTimeAdjustment(dwTimeAdjustment uint32, bTimeAdjustmentDisabled bool) error {
	_bTimeAdjustmentDisabled := win32.Bool32(bTimeAdjustmentDisabled)
	r1, _, e1 := syscall.SyscallN(procSetSystemTimeAdjustment.Addr(), uintptr(dwTimeAdjustment), uintptr(_bTimeAdjustmentDisabled))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// SetSystemTimeAdjustmentPrecise calls api-ms-win-core-sysinfo-l1-2-4!SetSystemTimeAdjustmentPrecise.
// https://learn.microsoft.com/windows/win32/api/sysinfoapi/nf-sysinfoapi-setsystemtimeadjustmentprecise
// Minimum OS: windows10.0.10240.
func SetSystemTimeAdjustmentPrecise(dwTimeAdjustment uint64, bTimeAdjustmentDisabled bool) error {
	_bTimeAdjustmentDisabled := win32.Bool32(bTimeAdjustmentDisabled)
	r1, _, e1 := syscall.SyscallN(procSetSystemTimeAdjustmentPrecise.Addr(), uintptr(dwTimeAdjustment), uintptr(_bTimeAdjustmentDisabled))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// VerSetConditionMask calls KERNEL32!VerSetConditionMask.
// https://learn.microsoft.com/windows/win32/api/winnt/nf-winnt-versetconditionmask
// Minimum OS: windows5.0.
func VerSetConditionMask(ConditionMask uint64, TypeMask VER_FLAGS, Condition byte) uint64 {
	r1, _, _ := syscall.SyscallN(procVerSetConditionMask.Addr(), uintptr(ConditionMask), uintptr(TypeMask), uintptr(Condition))
	return uint64(r1)
}

// VerifyVersionInfo calls KERNEL32!VerifyVersionInfoW.
// https://learn.microsoft.com/windows/win32/api/winbase/nf-winbase-verifyversioninfow
// Minimum OS: windows5.0.
func VerifyVersionInfo(lpVersionInformation *OSVERSIONINFOEXW, dwTypeMask VER_FLAGS, dwlConditionMask uint64) error {
	r1, _, e1 := syscall.SyscallN(procVerifyVersionInfo.Addr(), uintptr(unsafe.Pointer(lpVersionInformation)), uintptr(dwTypeMask), uintptr(dwlConditionMask))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}

// VerifyVersionInfoA calls KERNEL32!VerifyVersionInfoA.
// https://learn.microsoft.com/windows/win32/api/winbase/nf-winbase-verifyversioninfoa
// Minimum OS: windows5.0.
func VerifyVersionInfoA(lpVersionInformation *OSVERSIONINFOEXA, dwTypeMask VER_FLAGS, dwlConditionMask uint64) error {
	r1, _, e1 := syscall.SyscallN(procVerifyVersionInfoA.Addr(), uintptr(unsafe.Pointer(lpVersionInformation)), uintptr(dwTypeMask), uintptr(dwlConditionMask))
	if r1 == 0 {
		return win32.LastError(e1)
	}
	return nil
}
