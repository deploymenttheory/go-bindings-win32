...
// and writes the per-namespace ABI acceptance tests into testDir, pruning
// generated test files for namespaces that no longer produce any.
func runABITest(args []string) error {
	flags := flag.NewFlagSet("abitest", flag.ExitOnError)
	metadataDir := flags.String("metadata", filepath.Join("metadata", "win32"), "directory of .w32meta.json files")
	outDir := flags.String("out", filepath.Join("bindings", "win32"), "bindings output root")
	testDir := flags.String("test-dir", filepath.Join("acceptance", "abi"), "directory receiving the generated abi_*_test.go files")
	if err := flags.Parse(args); err != nil {
		return err
	}
	registry, err := pipeline.LoadAll(*metadataDir)
	...
	files := rawwin.BuildABITests(generator.ABIRecords(), generator.ImportPathFor)
	if err := os.MkdirAll(*testDir, 0o755); err != nil {
		return err
	}
	for name, source := range files {
		if err := os.WriteFile(filepath.Join(*testDir, name), []byte(source), 0o644); err != nil {
			return err
		}
	}
	pruned, err := pruneGeneratedTests(*testDir, files)
	if err != nil {
		return err
	}
	fmt.Printf("wrote %d ABI test files → %s (%d structs recorded, %d stale files pruned)\n",
		len(files), *testDir, len(generator.ABIRecords()), pruned)
	return nil
}

// pruneGeneratedTests deletes generated abi_*_test.go files in dir that this
// run did not write; hand-written files (no DO-NOT-EDIT header) are kept.
func pruneGeneratedTests(dir string, written map[string]string) (int, error) {
	...
	for _, entry := range entries {
		name := entry.Name()
		if entry.IsDir() || !strings.HasPrefix(name, "abi_") || !strings.HasSuffix(name, "_test.go") {
			continue
		}
		...
		if !strings.HasPrefix(string(content), "// Code generated by go-bindings-win32-codegen. DO NOT EDIT.") {
			continue
		}
