...
> schema remains a useful *cross-check trusted source* for the reader's output.
...
| Decision | Choice |
|----------|--------|
| **Input source** | Native Go ECMA-335 `.winmd` reader against the official `Microsoft.Windows.SDK.Win32Metadata` NuGet winmd. `win32json` = cross-check trusted source only. |
...
as its only input. Use the documented `win32json` schema as a **golden trusted source** —
...
into the IR. Gate it with the `win32json` golden-trusted source diff test. **This is the
...
- **winmd reader correctness** (the biggest net-new risk) — ECMA-335 table
  ...
  `win32json` golden trusted source; fuzz the blob decoder. The `windows-rs` and
