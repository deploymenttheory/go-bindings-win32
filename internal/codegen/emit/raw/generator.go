...
func (g *Generator) EmitAll(filter map[string]bool) (int, error) {
	...
	if len(filter) > 0 {
		...
		for _, meta := range g.registry.Namespaces {
			// External namespaces are loaded for type resolution only;
			// another module emits them.
			if pipeline.IsExternal(meta.Namespace) {
				continue
			}
	...
	for len(pending) > 0 {
		...
		// Chase namespaces this one referenced (externals resolve to another
		// module's packages and are never emitted here).
		var discovered []string
		for referenced := range g.mapper.Referenced {
			if !emitted[referenced] && !pipeline.IsExternal(referenced) {
...
// apiRootPrefix is the CLR namespace root the emitted doc comments cite
// ("Windows.Win32." here; "Windows.Wdk." in the wdk generator).
const apiRootPrefix = "Windows.Win32."
...
func (g *Generator) emitNamespace(meta *win32meta.NamespaceMeta) error {
	...
	// Handle closers file: Close<Handle>(h) error helpers from [RAIIFree].
	...
	doc := fmt.Sprintf(
		"%s\n\n//go:build %s\n\n// Package %s binds the %s%s API surface.\npackage %s\n",
		generatedHeader, fileasm.GeneratedBuildTag, packageName, apiRootPrefix, meta.Namespace, packageName)
